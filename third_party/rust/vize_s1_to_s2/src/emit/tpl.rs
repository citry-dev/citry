...
use vize_s0::String;
use vize_s2::op::{Attribute, BindingOp, DynamicName, IfBranch, Op};
...
fn should_unwrap_if(ops: &[Op<'_>], is_ts: bool, hoist_static: bool) -> bool {
    match ops {
        [Op::Element(element)] => {
            !element_has_own_key(element) && (!hoist_static || !is_hoistable(element, is_ts))
        }
        [Op::Component(component)] => !component_has_own_key(component),
        [Op::Slot(slot)] => !slot_has_own_key(slot),
        [Op::For(_)] => true,
        _ => false,
    }
}
...
pub(super) fn should_unwrap_for(ops: &[Op<'_>]) -> bool {
    matches!(ops, [Op::Element(element)]
        if !super::slots::is_slot_template(element) && !element_has_own_key(element))
}

fn element_has_own_key(element: &vize_s2::op::ElementOp<'_>) -> bool {
    element.attributes.iter().any(|attribute| attribute.name == "key")
        || element.bindings.iter().any(|binding| {
            matches!(binding, BindingOp::Bind(bind) if super::props_bind::is_key_bind_name(bind))
        })
}

fn component_has_own_key(component: &vize_s2::op::ComponentOp<'_>) -> bool {
    component
        .attributes
        .iter()
        .any(|attribute| attribute.name == "key")
        || component.bindings.iter().any(binding_has_static_key_name)
}

fn slot_has_own_key(slot: &vize_s2::op::SlotOp<'_>) -> bool {
    slot.attributes
        .iter()
        .any(|attribute| attribute.name == "key")
        || slot.bindings.iter().any(binding_has_static_key_name)
}

fn binding_has_static_key_name(binding: &BindingOp<'_>) -> bool {
    matches!(
        binding,

...
#[test]
fn a_keyed_component_template_v_if_child_keeps_branch_and_component_keys() {
    let code = assembled(
        r#"<template v-if="ok"><citry-child :key="preparedData.calls.callA.key"></citry-child></template>"#,
    );
    ...
    assert!(code.contains("key: preparedData.calls.callA.key"), "{code}");
}

#[test]
fn a_keyed_slot_template_v_if_child_keeps_branch_and_outlet_keys() {
    let code = assembled(r#"<template v-if="ok"><slot :key="outletKey"></slot></template>"#);
    ...
    assert!(
        code.contains("_renderSlot(_ctx.$slots, \"default\", { key: outletKey })"),
        "{code}"
    );
}

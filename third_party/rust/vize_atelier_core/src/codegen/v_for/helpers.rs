...
/// Check whether an element has either a static or dynamic key prop.
pub(crate) fn has_element_key(el: &ElementNode<'_>) -> bool {
    el.props.iter().any(|prop| match prop {
        PropNode::Attribute(attribute) => attribute.name == "key",
        PropNode::Directive(directive) => {
            directive.name == "bind"
                && matches!(directive.arg.as_ref(), Some(ExpressionNode::Simple(arg)) if arg.content == "key")
        }
    })
}

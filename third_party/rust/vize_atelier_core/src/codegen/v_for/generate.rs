...
use super::helpers::{get_element_key, has_element_key, has_other_props, should_skip_prop};
...
pub fn generate_for_item(ctx: &mut CodegenContext, node: &TemplateChildNode<'_>, is_stable: bool) {
    match node {
        TemplateChildNode::Element(el) => {
            ...
            // Keep a keyed single child inside the per-row Fragment. The
            // template key identifies the row; the child key can independently
            // identify a lifecycle revision within that row.
            let unwrapped_child =
                unwrap_template_single_element(el).filter(|child| !has_element_key(child));
            let gen_is_template = is_template && unwrapped_child.is_none();
            let directive_el = unwrapped_child.unwrap_or(el);
            ...
            let has_custom_dirs = has_custom_directives(directive_el);
            if has_custom_dirs {
            ...
            let has_vmodel = has_vmodel_directive(directive_el) && !has_custom_dirs;
            if has_vmodel {
                ctx.use_helper(RuntimeHelper::WithDirectives);
                ctx.push(ctx.helper(RuntimeHelper::WithDirectives));
                ctx.push("(");
            ...
            let has_vshow = has_vshow_directive(directive_el) && !has_vmodel && !has_custom_dirs;
            if has_vshow {
                ctx.use_helper(RuntimeHelper::WithDirectives);
            ...
            if has_custom_dirs {
                generate_custom_directives_closing(ctx, directive_el);
            }
            if has_vmodel {
                generate_vmodel_closing(ctx, directive_el);
            }
            if has_vshow {
                generate_vshow_closing(ctx, directive_el);
            }

...
class ContextMenuTouchAndPen(Component):
    template = """
      ...
      >
        ...
        >
          <c-fill name="target" data="{ target_attrs }">
            <button
              ...
              @click="touchState.targetClicks += 1"
              c-bind="target_attrs"
            >
        ...
        <div
          ...
          @scroll="
            touchState.phase='canceled: scroll';
            touchState.log.unshift('canceled scroll')
          "
        >
          <c-CContextMenu
            ...
            v-bind="{
              open:false,
              onOpenChange:(next,detail)=>{
                touchState.phase='controlled refused';
                touchState.log.unshift(`refused ${detail.reason}`);
                return false;
              },
            }"
          >
        ...
        <div class="context-menu-touch__controls">
          ...
          <button
            type="button"
            @click="touchState.log=[];touchState.phase='idle'"
          >Clear ledger</button>
          <output
            aria-live="polite"
            v-text="`State: ${touchState.phase};
              primary clicks: ${touchState.targetClicks}`"
          >State: idle; primary clicks: 0</output>
        </div>
        <ol aria-live="polite">
          <template v-for="entry in touchState.log.slice(0,5)" :key="entry">
            <li v-text="entry"></li>
          </template>
        </ol>
        <p>
    ...
    js = r"""
      $component({
        data(){return {touchState:{phase:'idle',log:[],targetClicks:0}};},
        methods: {
          simulate(kind) {

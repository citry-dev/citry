          ...
          <section class="citry-ui-quality-stack pin-input-quality" aria-labelledby="pin-input-states-title" >
            ...
            <form id="pin-input-quality-form" @submit.prevent="last=JSON.stringify(Array.from(new window.FormData($event.target).entries()))">
              <c-CField required>
                ...
                <c-fill name="default">
                  <c-CPinInput name="verification" value="01" c-attrs="{'data-quality-states':'required numeric partial form field description keyboard pointer paste autofill invalid-filter reset one-time-code'}" :onValueChange="(next,detail)=>last=`${detail.source}: ${next}`" :onComplete="(next)=>last=`Complete: ${next}`" />
                </c-fill>
              </c-CField>
              <button type="submit">Submit code</button><button type="reset">Reset code</button>
            </form>
            <div class="citry-ui-quality-grid">
              <c-CPinInput label="Controlled code" value="12" c-length="4" c-attrs="{'data-quality-states':'controlled callback refusal acceptance'}" :value="code" :onValueChange="(next)=>{code=next;last=`Accepted ${next}`}" />
            ...
            <output v-text="last">No PinInput action yet</output>
          </section>
        """
        js = """
          $component({
            data() {
              return {
                code:'12',last:'No PinInput action yet'
              };
            },
          });
        """

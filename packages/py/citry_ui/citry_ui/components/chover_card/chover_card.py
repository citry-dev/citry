"""Styled noninteractive HoverCard component family."""
...
CHoverCardSize = Literal["sm", "md", "lg"]


class CHoverCardOpenChangeDetail(TypedDict):
    reason: Literal[
        "hover",
        "focus",
        "pointer-leave",
...
class CHoverCardActivatorSlotData:
    activator_attrs: dict[str, object]
    hover_card_id: str


class CHoverCardDefaultSlotData:
    pass
...
_SURFACE_OWNED_ATTRS = frozenset(
    {
        ...
        "autofocus",
        "contenteditable",
        "data-citry-hover-card-exiting",
        "data-citry-hover-card-initialized",
        "data-citry-ui-part",
        "data-open",
        "data-arrow",
        "data-placement",
...
def _plain_optional_string(input_name: str, value: object) -> str | None:
    ...
    if not isinstance(raw, str):
        msg = f"CHoverCard {input_name} must be a string or None, got {raw!r}."
        raise TypeError(msg)
    plain = "".join(raw)
    if type(plain) is not str:
        msg = f"CHoverCard could not convert {input_name} to a plain string."
        raise TypeError(msg)
    return plain
...
def _plain_html_id(value: object) -> str | None:
    ...
    if not plain or any(character in "\t\n\f\r " for character in plain):
        msg = "CHoverCard id must be non-empty and cannot contain ASCII whitespace."
        raise ValueError(msg)
    if "\0" in plain:
        msg = "CHoverCard id cannot contain U+0000."
        raise ValueError(msg)
    return plain
...
def _plain_placement(value: object) -> str:
    ...
    if plain is None or plain not in _PLACEMENTS:
        ...
        msg = f"CHoverCard placement must be one of {expected}, got {plain!r}."
        raise ValueError(msg)
    return plain
...
def _milliseconds(input_name: str, value: object) -> int:
    ...
    if isinstance(raw, bool) or not isinstance(raw, int):
        msg = f"CHoverCard {input_name} must be an integer, got {raw!r}."
        raise TypeError(msg)
    if raw < 0 or raw > 60_000:
        msg = f"CHoverCard {input_name} must be between 0 and 60000, got {raw!r}."
        raise ValueError(msg)
    return raw
...
def _copy_attrs(attrs: Mapping[str, object] | None) -> dict[str, object]:
    ...
    if not isinstance(attrs, Mapping):
        msg = f"CHoverCard attrs must be a mapping or None, got {attrs!r}."
        raise TypeError(msg)
    return dict(attrs)
...
class CHoverCard(LibraryComponent):
    class Dependencies:
    ...
    class Kwargs:
        ...
        delay: int = 600
        close_delay: int = 300
        placement: CHoverCardPlacement = "bottom-start"
        arrow: bool = True
        size: CHoverCardSize = "md"
    ...
    @dataclass(slots=True)
    class Slots:
        activator: SlotInput[CHoverCardActivatorSlotData]
        default: SlotInput[CHoverCardDefaultSlotData]

    def _snapshot(self, kwargs: Kwargs) -> dict[str, object]:
        cached = getattr(self, "_cui_hover_card_snapshot", None)
        if cached is not None:
            return cached

        hover_card_id = _plain_html_id(kwargs.id) or f"cui-hover-card-{self.id}"
        if "default" not in self.raw_slots:
            msg = "CHoverCard requires a default fill."
            raise ValueError(msg)
        validate_boolean("CHoverCard", "open", kwargs.open)
        validate_boolean("CHoverCard", "disabled", kwargs.disabled)
        validate_boolean("CHoverCard", "arrow", kwargs.arrow)
        ...
        size = _plain_optional_string("size", kwargs.size)
        if size not in {"sm", "md", "lg"}:
            raise ValueError(f"CHoverCard size must be 'sm', 'md', or 'lg', got {size!r}.")
        attrs = _copy_attrs(kwargs.attrs)
        _validate_attrs(attrs)
        anchor_name = f"--_cui-hover-card-anchor-ref-{self.id}"
        generated_anchor_style = {"--_cui-hover-card-anchor": anchor_name}
        ...
        snapshot: dict[str, object] = {
            ...
            "size": size,
            "activator_attrs": {
                "data-citry-hover-card-trigger": "",
                "style": {"anchor-name": anchor_name},
            },
            "attrs": merge_root_attrs(attrs, kwargs.class_, surface_style),
        }
        self._cui_hover_card_snapshot = snapshot
        return snapshot
    ...
    template = """
      <div
        class="cui-hover-card-host"
        data-citry-hover-card-host
        data-citry-ui-part="host"
      >
        <c-slot
          name="activator"
          c-activator_attrs="activator_attrs"
          c-hover_card_id="hover_card_id"
          required
        />
        <div
          ...
          popover="manual"
          aria-hidden="true"
          data-citry-ui-part="hover-card"
        >
          <div data-citry-ui-part="content">
            <c-slot />
          </div>
          <span aria-hidden="true" data-citry-ui-part="arrow"></span>
        </div>
      </div>
    """
    ...
    js = (
        ANCHORED_LAYER_RUNTIME_JS
        + r"""
      const handoffKey = Symbol.for("citry-ui:hover-card-handoff");
      ...
      $component({
        ...
        init: ({ els, data, props, effect }) => {
          ...
          const surface = [...host.querySelectorAll('[data-citry-ui-part="hover-card"]')]
            .find((candidate) => nearestHost(candidate) === host);
          const triggers = [...host.querySelectorAll("[data-citry-hover-card-trigger]")]
          ...
          if (
            ...
            throw new Error(
              "[citry-ui] CHoverCard activator must spread activator_attrs onto exactly one HTMLElement.",
            );
          }
          ...
          if (
            ...
            throw new Error("[citry-ui] CHoverCard activator must be enabled and focusable.");
          }
          const anchorName = getComputedStyle(surface)
            .getPropertyValue("--_cui-hover-card-anchor")
            .trim();
          if (!anchorName.startsWith("--")) {
            throw new Error("[citry-ui] CHoverCard could not resolve its CSS anchor name.");
          }
          ...
          if (surface.querySelector(interactiveSelector)) {
            throw new Error("[citry-ui] CHoverCard content must be noninteractive.");
          }
          ...
          const reportInvalid = (name, value) => {
            ...
            console.error(
              `[citry-ui] CHoverCard ${name} received invalid client value `
          ...
          const duration = () => {
            const raw = getComputedStyle(surface)
              .getPropertyValue("--_cui-hover-card-duration")
              .trim();
            if (raw.endsWith("ms")) {
          ...
          const easing = () => getComputedStyle(surface)
            .getPropertyValue("--_cui-hover-card-easing")
            .trim() || "ease";
          const syncGeometry = () => {
            geometryFrame = null;
            if (!active || !logicalOpen || !surface.matches(":popover-open")) return;
            const triggerRect = trigger.getBoundingClientRect();
            const surfaceRect = surface.getBoundingClientRect();
            surface.dataset.side = surfaceRect.bottom <= triggerRect.top ? "top" : "bottom";
          };
          ...
          const startGeometry = () => {
            if (!geometryListening) {
              ...
              surface.ownerDocument.defaultView?.addEventListener("resize", scheduleGeometry);
              surface.ownerDocument.addEventListener("scroll", scheduleGeometry, true);
              surface.ownerDocument.defaultView?.visualViewport?.addEventListener("resize", scheduleGeometry);
              surface.ownerDocument.defaultView?.visualViewport?.addEventListener("scroll", scheduleGeometry);
            }
            scheduleGeometry();
          };
          const stopGeometry = () => {
            ...
            surface.ownerDocument.defaultView?.removeEventListener("resize", scheduleGeometry);
            surface.ownerDocument.removeEventListener("scroll", scheduleGeometry, true);
            surface.ownerDocument.defaultView?.visualViewport?.removeEventListener("resize", scheduleGeometry);
            surface.ownerDocument.defaultView?.visualViewport?.removeEventListener("scroll", scheduleGeometry);
          };
          const finishNativeClose = (currentGeneration) => {
            ...
            surface.removeAttribute("data-citry-hover-card-exiting");
          };
          ...
          const releaseHoverCardWarmth = () => {
            if (layerCoordinator.hoverCardLayer === layer) {
              layerCoordinator.hoverCardLayer = null;
              layerCoordinator.hoverCardWarmUntil = performance.now() + 300;
            }
          };
          const claimHoverCardWarmth = () => {
            if (layerCoordinator.hoverCardLayer && layerCoordinator.hoverCardLayer !== layer) {
              layerCoordinator.hoverCardLayer.requestPeerClose(surface);
            }
            layerCoordinator.hoverCardLayer = layer;
            layerCoordinator.hoverCardWarmUntil = Number.POSITIVE_INFINITY;
          };
          const applyOpen = (nextOpen, context = null) => {
            ...
            if (visible) {
              ...
              try {
                ...
                console.error("[citry-ui] CHoverCard could not enter the top layer:", error, surface);
                logicalOpen = false;
                internalOpen = false;
                return;
              }
            ...
            logicalOpen = false;
            surface.removeAttribute("data-open");
            surface.setAttribute("data-citry-hover-card-exiting", "");
            layerCoordinator.unregister(layer);
            releaseHoverCardWarmth();
            stopGeometry();
          ...
          const scheduleOpen = (reason, source) => {
            ...
            clearOpenTimer();
            const warmed = layerCoordinator.hoverCardWarmUntil === Number.POSITIVE_INFINITY
              || performance.now() <= layerCoordinator.hoverCardWarmUntil;
          ...
          const onToggle = (event) => {
            ...
            if (controlled && !configuration.disabled) {
              ...
              try {
                ...
                console.error(
                  "[citry-ui] CHoverCard could not restore controlled native visibility:",
                  error,
                  surface,
                );
              }
          ...
          effect(() => {
            ...
            configuration = {
              ...
              arrow: resolveBoolean("arrow"),
              size: (() => {
                const value = props.size === undefined ? data.size : props.size;
                if (["sm", "md", "lg"].includes(value)) {
                  invalidEpisodes.delete("size");
                  return value;
                }
                reportInvalid("size", value);
                return data.size;
              })(),
            };
            ...
            surface.toggleAttribute("data-arrow", configuration.arrow);
          ...
          host.setAttribute("data-citry-hover-card-initialized", "");
          ...
          return () => {
            ...
            surface.removeAttribute("data-open");
            surface.removeAttribute("data-citry-hover-card-exiting");
            host.removeAttribute("data-citry-hover-card-initialized");
          };
        },
      });
    """
    ...
    css = """
      @layer citry-ui.theme {
        :where(.cui-hover-card-host) {
          display: contents;
        }

        :where(.cui-hover-card) {
          --_cui-hover-card-background: var(
            --cui-hover-card-background,
            Canvas
          );
          --_cui-hover-card-foreground: var(
            --cui-hover-card-foreground,
            CanvasText
          );
          --_cui-hover-card-border-color: var(
            --cui-hover-card-border-color,
            color-mix(in srgb, currentColor 18%, transparent)
          );
          --_cui-hover-card-radius: var(--cui-hover-card-radius, 0.75rem);
          --_cui-hover-card-shadow: var(
            --cui-hover-card-shadow,
            0 1rem 2.5rem rgb(15 23 42 / 20%), 0 .25rem .75rem rgb(15 23 42 / 10%)
          );
          --_cui-hover-card-inline-size: var(--cui-hover-card-inline-size, 18rem);
          --_cui-hover-card-max-inline-size: var(--cui-hover-card-max-inline-size, 22rem);
          --_cui-hover-card-padding: var(--cui-hover-card-padding, .875rem);
          --_cui-hover-card-offset: var(--cui-hover-card-offset, 0.375rem);
          --_cui-hover-card-duration: var(--cui-hover-card-duration, 100ms);
          --_cui-hover-card-easing: var(
          ...
          position: fixed;
          position-anchor: var(--_cui-hover-card-anchor);
          position-try-fallbacks: flip-block, flip-inline, flip-block flip-inline;
          position-visibility: anchors-visible;
          display: block;
          inline-size: min(
            var(--_cui-hover-card-inline-size),
            calc(100dvi - 1rem)
          );
          max-inline-size: min(
            var(--_cui-hover-card-max-inline-size),
            calc(100dvi - 1rem)
          );
          margin: 0;
          padding: var(--_cui-hover-card-padding);
          overflow-wrap: anywhere;
          border: 1px solid var(--_cui-hover-card-border-color);
          border-radius: var(--_cui-hover-card-radius);
          background: var(--_cui-hover-card-background);
          color: var(--_cui-hover-card-foreground);
          box-shadow: var(--_cui-hover-card-shadow);
        ...
        :where(.cui-hover-card[data-size="sm"]) {
          --_cui-hover-card-inline-size: 15rem;
          --_cui-hover-card-padding: .75rem;
          font-size: .875rem;
        }

        :where(.cui-hover-card[data-size="lg"]) {
          --_cui-hover-card-inline-size: 21rem;
          --_cui-hover-card-padding: 1rem;
          font-size: 1rem;
        }

        :where(.cui-hover-card [data-citry-ui-part="content"]) {
          min-inline-size: 0;
        }

        :where(.cui-hover-card [data-citry-ui-part="arrow"]) {
          position: absolute;
          z-index: -1;
          inline-size: .75rem;
          block-size: .75rem;
        ...
        :where(.cui-hover-card:not([data-arrow]) [data-citry-ui-part="arrow"]) {
          display: none;
        }

        :where(.cui-hover-card[data-side="bottom"] [data-citry-ui-part="arrow"]) {
          inset-block-start: -.4375rem;
          border-inline-end: 0;
          border-block-end: 0;
        }

        :where(.cui-hover-card[data-side="top"] [data-citry-ui-part="arrow"]) {
        ...
        :where(
          .cui-hover-card:not(:popover-open):not([data-open]):not(
            [data-citry-hover-card-exiting]
          )
        ) {
          display: none;
        ...
        :where(.cui-hover-card-host:not([data-citry-hover-card-initialized]) .cui-hover-card) {
          display: none;
        }

        :where(.cui-hover-card[data-open]:not(:popover-open)) {
          position: static;
          display: block;
        ...
        :where(.cui-hover-card[data-citry-hover-card-exiting]) {
          pointer-events: none;
          user-select: none;
        }

        :where(.cui-hover-card[data-placement="bottom-start"]) {
          position-area: block-end span-inline-end;
          margin-block-start: var(--_cui-hover-card-offset);
        }

        :where(.cui-hover-card[data-placement="bottom"]) {
          position-area: block-end;
          margin-block-start: var(--_cui-hover-card-offset);
        }

        :where(.cui-hover-card[data-placement="bottom-end"]) {
          position-area: block-end span-inline-start;
          margin-block-start: var(--_cui-hover-card-offset);
        }

        :where(.cui-hover-card[data-placement="top-start"]) {
          position-area: block-start span-inline-end;
          margin-block-end: var(--_cui-hover-card-offset);
        }

        :where(.cui-hover-card[data-placement="top"]) {
          position-area: block-start;
          margin-block-end: var(--_cui-hover-card-offset);
        }

        :where(.cui-hover-card[data-placement="top-end"]) {

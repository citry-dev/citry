...
class DateRangeForm(Component):
    template = """
      <form @submit.prevent="result=JSON.stringify(Object.fromEntries(new window.FormData($event.target)))">
        ...
        <output v-text="result">Submit the form to inspect its canonical values.</output>
      </form>
    """
    js = "$component({data(){return {result:'Submit the form to inspect its canonical values.'};}});"

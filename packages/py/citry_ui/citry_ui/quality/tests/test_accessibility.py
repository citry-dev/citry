...
def test_every_axe_incomplete_disposition_has_automated_and_manual_ownership():
    assert set(AXE_INCOMPLETE_DISPOSITIONS) == {"color-contrast", "aria-valid-attr-value"}
    ...
    assert [item["rule"] for item in disposition_manifest()] == ["color-contrast", "aria-valid-attr-value"]

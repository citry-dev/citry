...
def _write_wheel(
    path: Path,
    *,
    dist_info: str = "citry_core-1.6.1.dist-info",
...
def test_expected_release_inventory_is_closed_and_includes_the_browser_wheel() -> None:
    names = expected_release_filenames("1.6.1")

    assert len(names) == 36
    assert "citry_core-1.6.1.tar.gz" in names
    assert "citry_core-1.6.1-cp314-cp314-pyemscripten_2026_0_wasm32.whl" in names
    ...
    assert "citry_core-1.6.1-cp310-abi3-manylinux_2_5_i686.manylinux1_i686.whl" in names
...
def test_package_version_falls_back_without_python_311_tomllib(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert distribution_verifier.package_version() == "1.6.1"
...
def test_pyodide_build_config_owns_the_exact_wheel_name() -> None:
    ...
    assert expected_wheel_name("1.6.1", config) == ("citry_core-1.6.1-cp314-cp314-pyemscripten_2026_0_wasm32.whl")
...
def test_wheel_verifier_checks_payload_metadata_extension_license_and_record(tmp_path: Path) -> None:
    wheel = tmp_path / "citry_core-1.6.1-cp314-cp314-pyemscripten_2026_0_wasm32.whl"
    _write_wheel(wheel)

    report = verify_wheel(wheel, version="1.6.1")
...
def test_wheel_verifier_accepts_the_canonical_abi3_extension(tmp_path: Path) -> None:
    wheel = tmp_path / "citry_core-1.6.1-cp310-abi3-macosx_11_0_arm64.whl"
    _write_wheel(
        wheel,
    ...
    windows_wheel = tmp_path / "citry_core-1.6.1-cp310-abi3-win_amd64.whl"
    _write_wheel(
        windows_wheel,
    ...
    windows_report = verify_wheel(windows_wheel, version="1.6.1")
...
def test_wheel_verifier_rejects_an_unchecked_python_payload(tmp_path: Path) -> None:
    ...
    with pytest.raises(DistributionVerificationError, match="Python payload differs"):
        verify_wheel(wheel, version="1.6.1")


def test_wheel_verifier_rejects_a_recorded_installer_script(tmp_path: Path) -> None:
    wheel = tmp_path / "citry_core-1.6.1-cp314-cp314-pyemscripten_2026_0_wasm32.whl"
    _write_wheel(wheel, extra_members={"citry_core-1.6.1.data/scripts/surprise": b"#!/bin/sh\n"})
...
def test_wheel_verifier_accepts_only_the_reviewed_musllinux_repair_library(
    ...
    wheel = tmp_path / "citry_core-1.6.1-cp314-cp314-musllinux_1_2_aarch64.whl"
...
def test_wheel_verifier_requires_the_canonical_metadata_directory(tmp_path: Path) -> None:
    wheel = tmp_path / "citry_core-1.6.1-cp314-cp314-pyemscripten_2026_0_wasm32.whl"
    _write_wheel(wheel, dist_info="other-1.6.1.dist-info")
...
def test_inventory_rejects_duplicate_basenames_before_staging(tmp_path: Path) -> None:
    ...
    for directory in (raw / "first", raw / "second"):
        (directory / "citry_core-1.6.1.tar.gz").write_bytes(b"duplicate")
...
def test_staged_bundle_requires_the_recorded_artifact_bytes(
    ...
    bundle.mkdir()
    wheel = bundle / "citry_core-1.6.1-cp310-abi3-test.whl"
    ...
    (bundle / "release-inventory.json").write_text(
        json.dumps({"version": "1.6.1", "artifacts": [artifact_report]}),
        encoding="utf-8",
    )
...
def test_promotion_checks_github_digest_before_safe_extraction(
    ...
    expected_report = {"version": "1.6.1", "artifacts": []}

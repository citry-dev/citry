...
def test_formatting_registration_is_scoped_to_each_workspace(tmp_path):
    ...
    assert isinstance(first_selector.pattern, types.RelativePattern)
    assert isinstance(second_selector.pattern, types.RelativePattern)
    assert first_selector.pattern.base_uri == first_workspace.as_uri()
    assert second_selector.pattern.base_uri == second_workspace.as_uri()
    assert first_selector.pattern.pattern == "**/*"
    assert second_selector.pattern.pattern == "**/*"
...
def test_expression_completion_downgrades_edits_for_older_clients(tmp_path):
    language_server = CitryLanguageServer()
    language_server.configure(
        types.InitializeParams(
            capabilities=types.ClientCapabilities(),
...
def test_structural_tag_completion_downgrades_snippets_for_older_clients(tmp_path):
    language_server = CitryLanguageServer()
    language_server.configure(
        types.InitializeParams(
            capabilities=types.ClientCapabilities(),
...
def test_attribute_completion_downgrades_atomic_edits_for_older_clients(tmp_path):
    language_server = CitryLanguageServer()
    language_server.configure(
        types.InitializeParams(
            capabilities=types.ClientCapabilities(),

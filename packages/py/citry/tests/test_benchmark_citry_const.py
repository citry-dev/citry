# The Const variant of the large citry scenario (test_benchmark_citry.py).
# It is the SAME port, with one difference: each component's template_data()
# wraps in Const() the values that are genuinely render-invariant (literal
...
# because a Const container yields non-Const elements when iterated and a
# Const dict yields non-Const values when subscripted, so the marker never
# reaches the loop bodies that do the rendering work. Marking only the true
# literals folds the small static regions that actually qualify.
#
# This file duplicates the plain variant by necessity (the benchmark harness
# slices each scenario file as a standalone string), so a change to any
# component's logic must be mirrored in both files.
#
# Faithfulness notes match the plain file (docs/design/benchmarking.md
# section 6.2): forms hand-written, Django-only helpers stubbed locally, DJC
# filters reimplemented as plain functions called from template_data.
...
def gen_render_data() -> dict:
    ...
    # No top-level Const wrapping: the project data is the per-render input, so
    # marking it Const would be a false promise (and useless, see the file
    # header). Const is applied inside template_data, on literals only.
    return data
...
class HeroIcon(Component):
    ...
    def template_data(self, kwargs, slots):
        ...
        # icon_paths is a slice of the module-level ICONS constant, so it is
        # the same on every render; default_attrs depends on the kwargs.
        return {"icon_paths": Const(icon_paths), "default_attrs": default_attrs, "attrs": kw.attrs}
...
class ExpansionPanel(Component):
    ...
    def template_data(self, kwargs, slots):
        return {
            ...
            "icon_attrs": Const({"style": "width: fit-content;", ":class": "{ 'rotate-180': isOpen }"}),
        }
...
class Tags(Component):
    ...
    def template_data(self, kwargs, slots):
        ...
        return {
            ...
            "attrs": kwargs.attrs,
            "x_props": x_props,
            "remove_btn_attrs": Const({"class": "!py-1", "@click": "removeTag(index)"}),
            "add_btn_attrs": Const({"class": "!py-1", "@click": "addTag"}),
            "has_title": bool(self.raw_slots.get("title")),
        }
...
class Bookmarks(Component):
    ...
    def template_data(self, kwargs, slots):
        ...
        return {
            ...
            "menu_items": Const(bookmark_menu_items),
            "attrs": kwargs.attrs,
            "theme": theme,
            "bookmark_icon_text_attrs": Const({"class": "py-2 text-sm"}),
            "plus_icon_text_attrs": Const({"class": "px-2 py-1 text-xs"}),
            "plus_icon_svg_attrs": Const({"class": "mt-0.5 ml-1"}),
            "menu_list_attrs": Const({"class": "w-24 ml-8 z-40"}),
            "menu_attrs": Const({"@click_outside": "onContextMenuClickOutside"}),
            "bookmark_js": Const({"onMenuToggle": "onContextMenuToggle"}),
        }
...
class Bookmark(Component):
    ...
    def template_data(self, kwargs, slots):
        ...
        return {
            ...
            "bookmark": bookmark,
            "x_props": x_props,
            "menu_icon_attrs": Const({"class": "self-center cursor-pointer", "x-ref": "bookmark_menu", "@click": "onMenuToggle"}),
            "menu_icon_svg_attrs": Const({"class": "inline"}),
            "menu_icon_text_attrs": Const({"class": "p-0"}),
        }
...
class ProjectUserAction(Component):
    ...
    def template_data(self, kwargs, slots):
        ...
        return {
            ...
            "icon_svg_attrs": Const({"class": "inline mb-1"}),
            "icon_attrs": Const({"class": "p-2", "@click.stop": "$dispatch('user_delete', { role })"}),
        }
...
class ProjectUsers(Component):
    ...
    def template_data(self, kwargs, slots):
        ...
        return {
            "editable": editable,
            "table_headers": Const(roles_table_headers),
            "table_rows": table_rows,
            "add_user_form": add_user_form,
            "submit_url": f"/submit/{project_id}/role/create",
            "project_url": f"/project/{project_id}",
            "table_attrs": Const({"@user_delete": "onUserDelete"}),
            "dialog_confirm_attrs": {":href": "role.delete_url"},
            "dialog_content_attrs": Const({"class": "w-full"}),
            "title_icon_attrs": Const({"class": "p-2 self-center"}),
        }
...
class ProjectOutputBadge(Component):
    ...
    def template_data(self, kwargs, slots):
        return {
            ...
            "check_interactive": Const(theme.check_interactive),
            "warn_icon_attrs": Const({"title": "A dependent dependency has not been met!"}),
            "check_icon_attrs": Const({"class": "p-2"}),
        }
...
class ProjectOutputAttachments(Component):
    ...
    def template_data(self, kwargs, slots):
        return {
            ...
            "text_max_len": Const(FORM_SHORT_TEXT_MAX_LEN),
            "tag_type": Const("project_output_attachment"),
            "preview_btn_attrs": Const(
                {
            ...
            "edit_btn_attrs": Const({"class": "!py-1", "x-text": "attachment.isPreview ? 'Edit' : 'Preview'", "@click": "() => $emit('toggleAttachment', index)"}),
            "remove_btn_attrs": Const({"class": "!py-1", "@click": "() => $emit('removeAttachment', index)"}),
            "tags_js_props": Const({"initTags": "attachment.tags", "onChange": "(tags) => $emit('setAttachmentTags', index, tags)"}),
            "tags_attrs": Const({"class": "pb-8"}),
        }
...
class ProjectOutputDependency(Component):
    ...
    def template_data(self, kwargs, slots):
        ...
        return {
            ...
            "placeholder": Const(OUTPUT_DESCRIPTION_PLACEHOLDER),
            "warn_icon_attrs": Const({"class": "float-left pr-1"}),
            "phase_btn_attrs": Const({"target": "_blank", "class": "hover:text-gray-600 !underline"}),
        }
...
class ProjectOutputForm(Component):
    ...
    def template_data(self, kwargs, slots):
        ...
        return {
            ...
            "placeholder": Const(OUTPUT_DESCRIPTION_PLACEHOLDER),
            "add_btn_attrs": Const({"@click": "addAttachment"}),
            "save_btn_attrs": Const({"@click": "onOutputSubmit({ reload: true })"}),
            "attach_js_props": {
...
class ProjectOutputs(Component):
    ...
    def template_data(self, kwargs, slots):
        ...
        return {
            ...
            "editable": kwargs.editable,
            "panel_attrs": Const({"class": "border-b border-solid border-gray-300 pb-2 mb-3"}),
            "panel_header_attrs": Const({"class": "flex align-center justify-between"}),
        }
...
class ProjectOutputsSummary(Component):
    ...
    def template_data(self, kwargs, slots):
        ...
        return {
            ...
            "editable": kwargs.editable,
            "groups": groups,
            "panel_header_attrs": Const({"class": "flex gap-x-2 prose"}),
        }
...
class ProjectInfo(Component):
    ...
    def template_data(self, kwargs, slots):
        ...
        return {
            ...
            "editable": kwargs.editable,
            "edit_btn_attrs": Const({"class": "not-prose"}),
        }
...
class Sidebar(Component):
    ...
    def template_data(self, kwargs, slots):
        ...
        return {
            ...
            "attrs": kwargs.attrs,
            "is_staff": is_staff,
            "sidebar_class": Const(theme.sidebar),
            "sidebar_link": Const(theme.sidebar_link),
            "faq_url": "/faq",
            "feedback_link_attrs": Const({"target": "_blank"}),
            "icon_text_attrs": Const({"class": "p-2"}),
            "child_btn_attrs": Const({"class": "p-2 !w-full"}),
        }
...
class Layout(Component):
    ...
    def template_data(self, kwargs, slots):
        return {
            ...
            "attrs": kwargs.attrs,
            "navbar_attrs": Const({"@sidebar_toggle": "toggleSidebar"}),
        }
...
class ProjectLayoutTabbed(Component):
    ...
    def template_data(self, kwargs, slots):
        ...
        return {
            ...
            "tabs_attrs": Const({"class": "p-6 h-full"}),
            "tabs_content_attrs": Const({"class": "flex flex-col"}),
        }
...
class ProjectPage(Component):
    ...
    def template_data(self, kwargs, slots):
        ...
        return {
            ...
            "phases_list_item_attrs": Const({"class": "py-5"}),
        }
...
# test: it guards that the Const-annotated port still renders the whole page
# (the Const markers must never change the output, only the work done).

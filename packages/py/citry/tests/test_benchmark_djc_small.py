# Vendored from django-components
# (https://github.com/django-components/django-components), file
# tests/test_benchmark_djc_small.py at commit
# cf999654459e9e1c255104c23fee386697c61d9a (2026-06-04, version 0.150.1).
# Only the pytest section below the "TESTS START" marker is adapted to this
# repo's conventions; the benchmarked code is unchanged.
# See docs/design/benchmarking.md.
#
...
# Adapted: upstream asserts against a syrupy snapshot; this repo locks the
# observed output into the assert directly (the "observe, then lock" rule).
# django-components tags the root element with a random per-render
# `data-djc-id-*` marker (upstream's `djc_test` fixture makes it
# deterministic); the assert strips it instead.

import re  # noqa: E402

_DJC_ID_RE = re.compile(r' data-djc-id-\w+=""')
...
def test_render():
    ...
    assert _DJC_ID_RE.sub("", rendered) == EXPECTED_HTML

...
(function () {
  ...
  var OWNERSHIP_COMMENT_PREFIX="citry:g1",REVISION_ALIAS_LENGTH=8,REVISION_RE=/^[0-9a-f]{64}$/,OWNERSHIP_COMMENT_RE=/^citry:g1:([0-9a-f]{8}):([0-9]+):([ir]):([0-9]+):([se])$/,ownershipRevisionAlias=r=>{if(!REVISION_RE.test(r))throw new TypeError("The client-graph revision must be lowercase SHA-256.");return r.slice(0,REVISION_ALIAS_LENGTH)},matchOwnershipComment=r=>OWNERSHIP_COMMENT_RE.exec(r.trim()),parseOwnershipComment=r=>{const e=matchOwnershipComment(r);if(e===null)return null;const[,i,c,n,s,o]=e;return{revisionAlias:i,graphId:c,kind:n,recordId:s,side:o,key:`${OWNERSHIP_COMMENT_PREFIX}:${i}:${c}:${n}:${s}`}},ProtocolValueError=class extends TypeError{constructor(r){super(r.message),this.name="ProtocolValueError",this.issue=r}},hasOwn=(r,e)=>Object.prototype.hasOwnProperty.call(r,e),pointer=(r,e)=>{const i=String(e).replace(/~/g,"~0").replace(/\//g,"~1");return r?`${r}/${i}`:`/${i}`},isPlainObject=r=>{if(r===null||typeof r!="object"||Array.isArray(r))return!1;const e=Object.getPrototypeOf(r);return e===Object.prototype||e===null},firstUnknown=(r,e)=>Object.keys(r).filter(i=>!e.has(i)).sort()[0]??null,containerIssue=(r,e)=>{if(Object.getOwnPropertySymbols(r).length)return{path:e,category:"strict_json",message:"The value contains a symbol-keyed property."};for(const i of Object.getOwnPropertyNames(r)){if(Array.isArray(r)&&i==="length")continue;const c=Object.getOwnPropertyDescriptor(r,i);if(!c?.enumerable||!("value"in c))return{path:pointer(e,i),category:"strict_json",message:"A JSON property must be an enumerable data property."}}return null},validateStrictJson=(r,e="")=>{const i=[{value:r,path:e,leaving:!1}],c=new Set;for(;i.length;){const n=i.pop(),s=n.value;if(n.leaving){c.delete(s);continue}if(s===null||typeof s=="string"||typeof s=="boolean")continue;if(typeof s=="number"){if(!Number.isFinite(s))return{path:n.path,category:"strict_json",message:"The value contains a non-finite number."};continue}if(typeof s!="object")return{path:n.path,category:"strict_json",message:"The value contains a non-JSON value."};if(!Array.isArray(s)&&!isPlainObject(s))return{path:n.path,category:"strict_json",message:"The value contains a non-JSON object."};const o=containerIssue(s,n.path);if(o)return o;if(c.has(s))return{path:n.path,category:"strict_json",message:"The value contains a cycle."};if(c.add(s),i.push({value:s,path:n.path,leaving:!0}),Array.isArray(s)){const l=Object.keys(s);if(l.length!==s.length||l.some((I,y)=>I!==String(y)))return{path:n.path,category:"strict_json",message:"A JSON array must be dense and carry no named properties."};for(let I=s.length-1;I>=0;I-=1)i.push({value:s[I],path:pointer(n.path,I),leaving:!1});continue}for(const l of Object.keys(s).sort().reverse())i.push({value:s[l],path:pointer(n.path,l),leaving:!1})}return null},PROTOCOL="citry-client-graph/1",MAX_SAFE_INTEGER=Number.MAX_SAFE_INTEGER,canonicalValue=r=>{if(r===null||typeof r=="string"||typeof r=="boolean")return JSON.stringify(r);if(typeof r=="number"){if(!Number.isSafeInteger(r)||r<0)throw new TypeError("client-graph numbers must be non-negative safe integers");return String(r)}if(Array.isArray(r))return`[${r.map(canonicalValue).join(",")}]`;if(isPlainObject(r))return`{${Object.keys(r).sort().map(e=>`${JSON.stringify(e)}:${canonicalValue(r[e])}`).join(",")}}`;throw new TypeError("unsupported client-graph JSON value")},sha256=r=>{const e=new TextEncoder().encode(r),i=Math.ceil((e.length+9)/64)*64,c=new Uint8Array(i);c.set(e),c[e.length]=128;const n=new DataView(c.buffer);n.setUint32(i-8,Math.floor(e.length/536870912),!1),n.setUint32(i-4,e.length<<3>>>0,!1);const s=new Uint32Array([1116352408,1899447441,3049323471,3921009573,961987163,1508970993,2453635748,2870763221,3624381080,310598401,607225278,1426881987,1925078388,2162078206,2614888103,3248222580,3835390401,4022224774,264347078,604807628,770255983,1249150122,1555081692,1996064986,2554220882,2821834349,2952996808,3210313671,3336571891,3584528711,113926993,338241895,666307205,773529912,1294757372,1396182291,1695183700,1986661051,2177026350,2456956037,2730485921,2820302411,3259730800,3345764771,3516065817,3600352804,4094571909,275423344,430227734,506948616,659060556,883997877,958139571,1322822218,1537002063,1747873779,1955562222,2024104815,2227730452,2361852424,2428436474,2756734187,3204031479,3329325298]),o=new Uint32Array([1779033703,3144134277,1013904242,2773480762,1359893119,2600822924,528734635,1541459225]),l=new Uint32Array(64),I=(y,p)=>y>>>p|y<<32-p;for(let y=0;y<i;y+=64){for(let m=0;m<16;m+=1)l[m]=n.getUint32(y+m*4,!1);for(let m=16;m<64;m+=1){const $=l[m-15],k=l[m-2],O=I($,7)^I($,18)^$>>>3,a=I(k,17)^I(k,19)^k>>>10;l[m]=l[m-16]+O+l[m-7]+a>>>0}let p=o[0],g=o[1],R=o[2],C=o[3],v=o[4],L=o[5],w=o[6],b=o[7];for(let m=0;m<64;m+=1){const $=I(v,6)^I(v,11)^I(v,25),k=v&L^~v&w,O=b+$+k+s[m]+l[m]>>>0,a=I(p,2)^I(p,13)^I(p,22),t=p&g^p&R^g&R,d=a+t>>>0;b=w,w=L,L=v,v=C+O>>>0,C=R,R=g,g=p,p=O+d>>>0}o[0]=o[0]+p>>>0,o[1]=o[1]+g>>>0,o[2]=o[2]+R>>>0,o[3]=o[3]+C>>>0,o[4]=o[4]+v>>>0,o[5]=o[5]+L>>>0,o[6]=o[6]+w>>>0,o[7]=o[7]+b>>>0}return Array.from(o).map(y=>y.toString(16).padStart(8,"0")).join("")},revisionFor=r=>sha256(canonicalValue(r)),revisionForManifest=r=>{const e={};for(const i of Object.keys(r))i!=="revision"&&(e[i]=r[i]);return revisionFor(e)},RENDER_ID=/^[a-z0-9_-]+$/,LOCATION_KINDS=new Set(["component-call","component-tag-client-binding","implicit-fill","named-fill","fallback-fill","slot-outlet"]),BINDING_SOURCES=new Set(["direct","server-dynamic","spread"]),FILL_KINDS=new Set(["implicit","named","fallback","python","typed-default"]),SOURCE_POLICIES=new Set(["template","python-detached","typed-default-detached"]),MORPH_MODES=new Set(["ignore"]),COMPONENT_CLASS_FIELDS=["classId","className"],COMPONENT_INSTANCE_FIELDS=["instanceId","renderId","classId","invocationId","parentRenderId","transparent"],SOURCE_LOCATION_FIELDS=["locationId","kind","ownerRenderId","ownerClassId","carrierInstanceId","origin","sourceOffset","sourcePos","mappingKey","mappingIndex"],EXPRESSION_PAYLOAD_FIELDS=["type","expression"],DOM_PAYLOAD_FIELDS=["type","classId","event","handler","args","prevent","stop","self","once","key","debounce","throttle"],POLL_PAYLOAD_FIELDS=["type","classId","handler","args","interval"],CLIENT_BINDING_FIELDS=["key","source","locationId","payload"],NESTED_COMPONENT_FIELDS=["invocationId","sourceRenderId","sourceClassId","locationId","tagName","targetClassId","morphKey","morphMode","targetRenderId","parentRegionId","clientBindings"],EXECUTION_CONSTRAINT_FIELDS=["invocationId","parentRenderId","childRenderId"],FILL_FIELDS=["fillId","kind","slotName","policy","ownerRenderId","ownerClassId","locationId","sourceInvocationId","receiverRenderId","receiverClassId","fallbackLocationId"],SLOT_REGION_FIELDS=["regionId","fillId","receiverRenderId","slotLocationId","ownerRenderId","sourceLocationId","parentRegionId","transitionFromRenderId","resultOwnerRenderId"],GRAPH_FIELDS=["graphId","componentClasses","componentInstances","sourceLocations","nestedComponents","componentExecutionOrderConstraints","fills","slotRegions"],recordIssue=(r,e,i,c,n)=>{if(n){const o=validateStrictJson(r,e);if(o)return o}if(!isPlainObject(r))return{path:e,category:"type",message:`${c} must be an object.`};for(const o of i)if(!hasOwn(r,o))return{path:pointer(e,o),category:"required",message:`${c} requires '${o}'.`};const s=firstUnknown(r,new Set(i));return s===null?null:{path:pointer(e,s),category:"unknown_field",message:`${c} has an unknown field.`}},stringIssue=(r,e,i)=>typeof r=="string"?null:{path:e,category:"type",message:`${i} must be a string.`},nullableStringIssue=(r,e,i)=>r===null||typeof r=="string"?null:{path:e,category:"type",message:`${i} must be a string or null.`},integerIssue=(r,e,i,c)=>typeof r!="number"||!Number.isInteger(r)?{path:e,category:typeof r=="number"&&!Number.isFinite(r)?"strict_json":"type",message:`${i} must be an integer.`}:!Number.isSafeInteger(r)||r<c?{path:e,category:"range",message:`${i} is outside the client-graph range.`}:null,nullableIntegerIssue=(r,e,i,c)=>r===null?null:integerIssue(r,e,i,c),enumIssue=(r,e,i,c)=>typeof r!="string"?{path:e,category:"type",message:`${c} must be a string.`}:i.has(r)?null:{path:e,category:"enum",message:`${c} is not a client-graph v1 value.`},nullableEnumIssue=(r,e,i,c)=>r===null?null:enumIssue(r,e,i,c),validateComponentClass=(r,e="",i=!0)=>{const c=recordIssue(r,e,COMPONENT_CLASS_FIELDS,"A component-class record",i);if(c)return c;const n=r;for(const s of COMPONENT_CLASS_FIELDS){const o=stringIssue(n[s],pointer(e,s),`The component ${s}`);if(o)return o}return null},validateComponentInstance=(r,e="",i=!0)=>{const c=recordIssue(r,e,COMPONENT_INSTANCE_FIELDS,"A component-instance record",i);if(c)return c;const n=r,s=[integerIssue(n.instanceId,pointer(e,"instanceId"),"The instance ID",1),stringIssue(n.renderId,pointer(e,"renderId"),"The render ID"),stringIssue(n.classId,pointer(e,"classId"),"The class ID"),nullableIntegerIssue(n.invocationId,pointer(e,"invocationId"),"The invocation ID",1),nullableStringIssue(n.parentRenderId,pointer(e,"parentRenderId"),"The parent render ID")];for(const o of s)if(o)return o;return RENDER_ID.test(n.renderId)?typeof n.transparent=="boolean"?null:{path:pointer(e,"transparent"),category:"type",message:"The transparent flag must be a boolean."}:{path:pointer(e,"renderId"),category:"pattern",message:"The component renderId is not safe for an HTML attribute name."}},validateSourceLocation=(r,e="",i=!0)=>{const c=recordIssue(r,e,SOURCE_LOCATION_FIELDS,"A source-location record",i);if(c)return c;const n=r,s=[integerIssue(n.locationId,pointer(e,"locationId"),"The location ID",1),enumIssue(n.kind,pointer(e,"kind"),LOCATION_KINDS,"The location kind"),stringIssue(n.ownerRenderId,pointer(e,"ownerRenderId"),"The location owner render ID"),stringIssue(n.ownerClassId,pointer(e,"ownerClassId"),"The location owner class ID"),integerIssue(n.carrierInstanceId,pointer(e,"carrierInstanceId"),"The carrier instance ID",1),nullableStringIssue(n.origin,pointer(e,"origin"),"The source origin")];for(const g of s)if(g)return g;const o=pointer(e,"sourceOffset");let l=recordIssue(n.sourceOffset,o,["start","end"],"A source-offset record",!1);if(l)return l;const I=n.sourceOffset;for(const g of["start","end"])if(l=integerIssue(I[g],pointer(o,g),`The source-offset ${g}`,0),l)return l;const y=pointer(e,"sourcePos");if(l=recordIssue(n.sourcePos,y,["line","column"],"A source-position record",!1),l)return l;const p=n.sourcePos;for(const g of["line","column"])if(l=integerIssue(p[g],pointer(y,g),`The source-position ${g}`,1),l)return l;return l=nullableStringIssue(n.mappingKey,pointer(e,"mappingKey"),"The mapping key"),l||nullableIntegerIssue(n.mappingIndex,pointer(e,"mappingIndex"),"The mapping index",0)},validateClientBindingPayload=(r,e="",i=!0)=>{if(i){const o=validateStrictJson(r,e);if(o)return o}if(!isPlainObject(r))return{path:e,category:"type",message:"A client-binding payload must be an object."};if(!hasOwn(r,"type"))return{path:pointer(e,"type"),category:"required",message:"A client-binding payload requires 'type'."};if(typeof r.type!="string")return{path:pointer(e,"type"),category:"type",message:"The client-binding payload type must be a string."};const c=r.type==="props"||r.type==="alpine-handler"?EXPRESSION_PAYLOAD_FIELDS:r.type==="citry-dom-event"?DOM_PAYLOAD_FIELDS:r.type==="citry-poll"?POLL_PAYLOAD_FIELDS:null;if(c===null)return{path:pointer(e,"type"),category:"enum",message:"The client-binding payload type is not a v1 value."};const n=recordIssue(r,e,c,"A client-binding payload",!1);if(n)return n;if(r.type==="props"||r.type==="alpine-handler")return stringIssue(r.expression,pointer(e,"expression"),"The Alpine expression");if(r.type==="citry-poll"){for(const l of["classId","handler"]){const I=stringIssue(r[l],pointer(e,l),`The poll ${l}`);if(I)return I}return nullableStringIssue(r.args,pointer(e,"args"),"The poll arguments")??integerIssue(r.interval,pointer(e,"interval"),"The poll interval",1)}for(const o of["classId","event","handler"]){const l=stringIssue(r[o],pointer(e,o),`The DOM-event ${o}`);if(l)return l}let s=nullableStringIssue(r.args,pointer(e,"args"),"The DOM-event args");if(s)return s;for(const o of["prevent","stop","self","once"])if(typeof r[o]!="boolean")return{path:pointer(e,o),category:"type",message:`The DOM-event ${o} flag must be a boolean.`};if(s=nullableStringIssue(r.key,pointer(e,"key"),"The DOM-event key"),s)return s;for(const o of["debounce","throttle"])if(s=nullableIntegerIssue(r[o],pointer(e,o),`The DOM-event ${o} delay`,0),s)return s;return null},validateClientBinding=(r,e="",i=!0)=>{const c=recordIssue(r,e,CLIENT_BINDING_FIELDS,"A component-tag client-binding record",i);if(c)return c;const n=r,s=[stringIssue(n.key,pointer(e,"key"),"The client-binding key"),enumIssue(n.source,pointer(e,"source"),BINDING_SOURCES,"The client-binding source"),nullableIntegerIssue(n.locationId,pointer(e,"locationId"),"The client-binding location ID",1)];for(const o of s)if(o)return o;return validateClientBindingPayload(n.payload,pointer(e,"payload"),!1)},validateNestedComponent=(r,e="",i=!0)=>{const c=recordIssue(r,e,NESTED_COMPONENT_FIELDS,"A nested-component record",i);if(c)return c;const n=r,s=[integerIssue(n.invocationId,pointer(e,"invocationId"),"The invocation ID",1),stringIssue(n.sourceRenderId,pointer(e,"sourceRenderId"),"The source render ID"),stringIssue(n.sourceClassId,pointer(e,"sourceClassId"),"The source class ID"),nullableIntegerIssue(n.locationId,pointer(e,"locationId"),"The location ID",1),stringIssue(n.tagName,pointer(e,"tagName"),"The nested-component tag name"),stringIssue(n.targetClassId,pointer(e,"targetClassId"),"The target class ID"),nullableStringIssue(n.morphKey,pointer(e,"morphKey"),"The component morph key"),nullableEnumIssue(n.morphMode,pointer(e,"morphMode"),MORPH_MODES,"The component morph mode"),stringIssue(n.targetRenderId,pointer(e,"targetRenderId"),"The target render ID"),nullableIntegerIssue(n.parentRegionId,pointer(e,"parentRegionId"),"The parent slot-region ID",1)];for(const o of s)if(o)return o;if(!Array.isArray(n.clientBindings))return{path:pointer(e,"clientBindings"),category:"type",message:"Client bindings must be an array."};for(let o=0;o<n.clientBindings.length;o+=1){const l=validateClientBinding(n.clientBindings[o],pointer(pointer(e,"clientBindings"),o),!1);if(l)return l}return null},validateExecutionConstraint=(r,e="",i=!0)=>{const c=recordIssue(r,e,EXECUTION_CONSTRAINT_FIELDS,"An execution-order constraint",i);if(c)return c;const n=r;return integerIssue(n.invocationId,pointer(e,"invocationId"),"The invocation ID",1)??stringIssue(n.parentRenderId,pointer(e,"parentRenderId"),"The parent render ID")??stringIssue(n.childRenderId,pointer(e,"childRenderId"),"The child render ID")},validateFill=(r,e="",i=!0)=>{const c=recordIssue(r,e,FILL_FIELDS,"A fill record",i);if(c)return c;const n=r;return[integerIssue(n.fillId,pointer(e,"fillId"),"The fill ID",1),enumIssue(n.kind,pointer(e,"kind"),FILL_KINDS,"The fill kind"),stringIssue(n.slotName,pointer(e,"slotName"),"The fill slot name"),enumIssue(n.policy,pointer(e,"policy"),SOURCE_POLICIES,"The fill source policy"),nullableStringIssue(n.ownerRenderId,pointer(e,"ownerRenderId"),"The fill owner render ID"),nullableStringIssue(n.ownerClassId,pointer(e,"ownerClassId"),"The fill owner class ID"),nullableIntegerIssue(n.locationId,pointer(e,"locationId"),"The fill location ID",1),nullableIntegerIssue(n.sourceInvocationId,pointer(e,"sourceInvocationId"),"The source invocation ID",1),nullableStringIssue(n.receiverRenderId,pointer(e,"receiverRenderId"),"The fill receiver render ID"),nullableStringIssue(n.receiverClassId,pointer(e,"receiverClassId"),"The fill receiver class ID"),nullableIntegerIssue(n.fallbackLocationId,pointer(e,"fallbackLocationId"),"The fallback location ID",1)].find(o=>o!==null)??null},validateSlotRegion=(r,e="",i=!0)=>{const c=recordIssue(r,e,SLOT_REGION_FIELDS,"A slot-region record",i);if(c)return c;const n=r;return[integerIssue(n.regionId,pointer(e,"regionId"),"The slot-region ID",1),integerIssue(n.fillId,pointer(e,"fillId"),"The fill ID",1),nullableStringIssue(n.receiverRenderId,pointer(e,"receiverRenderId"),"The receiver render ID"),nullableIntegerIssue(n.slotLocationId,pointer(e,"slotLocationId"),"The slot location ID",1),nullableStringIssue(n.ownerRenderId,pointer(e,"ownerRenderId"),"The owner render ID"),nullableIntegerIssue(n.sourceLocationId,pointer(e,"sourceLocationId"),"The source location ID",1),nullableIntegerIssue(n.parentRegionId,pointer(e,"parentRegionId"),"The parent slot-region ID",1),nullableStringIssue(n.transitionFromRenderId,pointer(e,"transitionFromRenderId"),"The transition source render ID"),nullableStringIssue(n.resultOwnerRenderId,pointer(e,"resultOwnerRenderId"),"The result owner render ID")].find(o=>o!==null)??null},validateGraph=(r,e="",i=!0)=>{const c=recordIssue(r,e,GRAPH_FIELDS,"A graph record",i);if(c)return c;const n=r,s=integerIssue(n.graphId,pointer(e,"graphId"),"The graph ID",0);if(s)return s;const o=[["componentClasses",validateComponentClass],["componentInstances",validateComponentInstance],["sourceLocations",validateSourceLocation],["nestedComponents",validateNestedComponent],["componentExecutionOrderConstraints",validateExecutionConstraint],["fills",validateFill],["slotRegions",validateSlotRegion]];for(const[l,I]of o){const y=n[l],p=pointer(e,l);if(!Array.isArray(y))return{path:p,category:"type",message:`The graph's ${l} must be an array.`};for(let g=0;g<y.length;g+=1){const R=I(y[g],pointer(p,g),!1);if(R)return R}}return null},cycle=r=>{const e=new Set,i=new Set;for(const c of r.keys()){let n=c;const s=[];for(;n!=null&&r.has(n)&&!i.has(n);){if(e.has(n))return!0;e.add(n),s.push(n);const o=r.get(n);if(o===void 0)break;n=o}for(const o of s)e.delete(o),i.add(o)}return!1},executionCycle=r=>{const e=new Set,i=new Set,c=Array.from(r.keys()).reverse().map(n=>[n,!1]);for(;c.length;){const[n,s]=c.pop();if(s){e.delete(n),i.add(n);continue}if(e.has(n))return!0;if(!i.has(n)){e.add(n),c.push([n,!0]);for(const o of[...r.get(n)??[]].reverse())c.push([o,!1])}}return!1},semantic=(r,e)=>({path:r,category:"semantic",message:e}),bindingKeyIssue=(r,e,i)=>{if(r.type==="props"&&e!=="$c-props")return semantic(i,"A props client binding must use the $c-props key.");if(r.type==="alpine-handler"&&!(e.startsWith("@")&&!e.startsWith("@c-")||e.startsWith("x-on:")))return semantic(i,"An Alpine-handler client binding has a non-Alpine key.");if(r.type==="citry-dom-event"){if(!e.startsWith("@c-")||e.slice(3).split(".")[0]==="poll")return semantic(i,"A Citry DOM-event client binding has a non-event key.");if(e.slice(3).split(".")[0]!==r.event)return semantic(i,"A Citry DOM-event client binding disagrees with its key.")}return r.type==="citry-poll"&&!e.startsWith("@c-poll.")?semantic(i,"A Citry poll client binding must use an @c-poll key."):null},validateRelationships=(r,e="")=>{const i=r.mode==="development",c=new Set;for(let n=0;n<r.graphs.length;n+=1){const s=r.graphs[n],o=pointer(pointer(e,"graphs"),n);if(s.graphId!==n)return semantic(pointer(o,"graphId"),`graphs[${n}].graphId is not dense and ordered.`);const l=[["componentInstances","instanceId"],["sourceLocations","locationId"],["nestedComponents","invocationId"],["fills","fillId"],["slotRegions","regionId"]],I=new Map;for(const[a,t]of l){const f=s[a].map(h=>h[t]),u=new Set(f);if(I.set(a,u),f.length!==u.size)return semantic(pointer(o,a),`graphs[${n}].${a} has duplicate ids.`)}if(!i){if(s.sourceLocations.length)return semantic(pointer(o,"sourceLocations"),`graphs[${n}] production manifest has sourceLocations.`);for(let a=0;a<s.nestedComponents.length;a+=1){const t=s.nestedComponents[a],d=pointer(pointer(o,"nestedComponents"),a);if(t.locationId!==null)return semantic(pointer(d,"locationId"),`graphs[${n}] production invocation has a location reference.`);for(let f=0;f<t.clientBindings.length;f+=1)if(t.clientBindings[f].locationId!==null){const u=pointer(pointer(d,"clientBindings"),f);return semantic(pointer(u,"locationId"),`graphs[${n}] production client binding has a location reference.`)}}for(let a=0;a<s.fills.length;a+=1){const t=s.fills[a];for(const d of["locationId","fallbackLocationId"])if(t[d]!==null)return semantic(pointer(pointer(pointer(o,"fills"),a),d),`graphs[${n}] production fill has a location reference.`)}for(let a=0;a<s.slotRegions.length;a+=1){const t=s.slotRegions[a];for(const d of["slotLocationId","sourceLocationId"])if(t[d]!==null)return semantic(pointer(pointer(pointer(o,"slotRegions"),a),d),`graphs[${n}] production slot region has a location reference.`)}}const y=new Set;for(let a=0;a<s.componentClasses.length;a+=1){const t=s.componentClasses[a].classId;if(y.has(t))return semantic(pointer(pointer(pointer(o,"componentClasses"),a),"classId"),`graphs[${n}] has duplicate class ids.`);y.add(t)}const p=new Set,g=new Map,R=new Map,C=new Map,v=[],L=new Map;for(let a=0;a<s.componentInstances.length;a+=1){const t=s.componentInstances[a],d=pointer(pointer(o,"componentInstances"),a);if(!y.has(t.classId))return semantic(pointer(d,"classId"),`graphs[${n}] component instance classId is unknown.`);if(p.has(t.renderId))return semantic(pointer(d,"renderId"),`graphs[${n}] has duplicate render ids.`);if(c.has(t.renderId))return semantic(pointer(d,"renderId"),`render id '${t.renderId}' appears in more than one graph.`);p.add(t.renderId),c.add(t.renderId),g.set(t.renderId,t.classId),R.set(t.instanceId,t.renderId),L.set(t.renderId,t.parentRenderId);const f=[t.renderId,t.parentRenderId,t.invocationId];if(v.push(f),t.invocationId!==null){const u=C.get(t.invocationId)??[];u.push(f),C.set(t.invocationId,u)}}for(let a=0;a<s.componentInstances.length;a+=1){const t=s.componentInstances[a].parentRenderId;if(t!==null&&!p.has(t))return semantic(pointer(pointer(pointer(o,"componentInstances"),a),"parentRenderId"),`graphs[${n}] component instance parentRenderId is unknown.`)}const w=new Map,b=new Map;if(i)for(let a=0;a<s.sourceLocations.length;a+=1){const t=s.sourceLocations[a],d=pointer(pointer(o,"sourceLocations"),a);if(!I.get("componentInstances").has(t.carrierInstanceId))return semantic(pointer(d,"carrierInstanceId"),`graphs[${n}] location has an unknown carrier.`);if(t.sourceOffset.start>t.sourceOffset.end)return semantic(pointer(d,"sourceOffset"),`graphs[${n}] location has a reversed byte range.`);if(!p.has(t.ownerRenderId)||g.get(t.ownerRenderId)!==t.ownerClassId)return semantic(pointer(d,"ownerRenderId"),`graphs[${n}] location owner is unknown or mismatched.`);if(R.get(t.carrierInstanceId)!==t.ownerRenderId)return semantic(pointer(d,"carrierInstanceId"),`graphs[${n}] location carrier is mismatched.`);w.set(t.locationId,[t.ownerRenderId,t.ownerClassId]),b.set(t.locationId,t.kind)}const m=new Map;for(let a=0;a<s.nestedComponents.length;a+=1){const t=s.nestedComponents[a],d=pointer(pointer(o,"nestedComponents"),a);if(!p.has(t.sourceRenderId)||g.get(t.sourceRenderId)!==t.sourceClassId)return semantic(pointer(d,"sourceRenderId"),`graphs[${n}] invocation source is unknown or mismatched.`);if(!p.has(t.targetRenderId)||g.get(t.targetRenderId)!==t.targetClassId)return semantic(pointer(d,"targetRenderId"),`graphs[${n}] invocation target is unknown or mismatched.`);if(i){if(!I.get("sourceLocations").has(t.locationId))return semantic(pointer(d,"locationId"),`graphs[${n}] invocation has an unknown location.`);const f=w.get(t.locationId);if(!f||f[0]!==t.sourceRenderId||f[1]!==t.sourceClassId)return semantic(pointer(d,"locationId"),`graphs[${n}] invocation location owner is mismatched.`);if(b.get(t.locationId)!=="component-call")return semantic(pointer(d,"locationId"),`graphs[${n}] invocation location kind is mismatched.`)}if(t.parentRegionId!==null&&!I.get("slotRegions").has(t.parentRegionId))return semantic(pointer(d,"parentRegionId"),`graphs[${n}] nested component parentRegionId references an unknown slot region.`);m.set(t.invocationId,[t.sourceRenderId,t.targetRenderId]);for(let f=0;f<t.clientBindings.length;f+=1){const u=t.clientBindings[f],h=pointer(pointer(d,"clientBindings"),f);if(i){if(!I.get("sourceLocations").has(u.locationId))return semantic(pointer(h,"locationId"),`graphs[${n}] client binding has an unknown location.`);const S=w.get(u.locationId);if(!S||S[0]!==t.sourceRenderId||S[1]!==t.sourceClassId)return semantic(pointer(h,"locationId"),`graphs[${n}] client-binding location owner is mismatched.`);if(b.get(u.locationId)!=="component-tag-client-binding")return semantic(pointer(h,"locationId"),`graphs[${n}] client-binding location kind is mismatched.`)}if((u.payload.type==="citry-dom-event"||u.payload.type==="citry-poll")&&u.payload.classId!==t.sourceClassId)return semantic(pointer(pointer(h,"payload"),"classId"),`graphs[${n}] Citry client-binding class is not its source parent.`);const T=bindingKeyIssue(u.payload,u.key,pointer(h,"key"));if(T)return T}}for(let a=0;a<s.componentInstances.length;a+=1){const t=s.componentInstances[a].invocationId;if(t!==null&&!I.get("nestedComponents").has(t))return semantic(pointer(pointer(pointer(o,"componentInstances"),a),"invocationId"),`graphs[${n}] instance has an unknown invocation.`)}for(let a=0;a<v.length;a+=1){const[t,d,f]=v[a],u=pointer(pointer(o,"componentInstances"),a);if(f===null){if(d!==null)return semantic(pointer(u,"parentRenderId"),`graphs[${n}] uninvoked instance has a parent.`);continue}const h=m.get(f);if(!h||h[0]!==d||h[1]!==t)return semantic(pointer(u,"invocationId"),`graphs[${n}] instance endpoints do not match their invocation.`)}for(let a=0;a<s.nestedComponents.length;a+=1){const t=s.nestedComponents[a].invocationId;if((C.get(t)??[]).length!==1)return semantic(pointer(pointer(pointer(o,"nestedComponents"),a),"invocationId"),`graphs[${n}] invocation does not bind exactly one target instance.`)}const $=new Map;for(let a=0;a<s.fills.length;a+=1){const t=s.fills[a],d=pointer(pointer(o,"fills"),a);if(t.ownerRenderId===null!=(t.ownerClassId===null)||t.ownerRenderId!==null&&g.get(t.ownerRenderId)!==t.ownerClassId)return semantic(pointer(d,"ownerRenderId"),`graphs[${n}] fill owner and class are mismatched.`);if(t.receiverRenderId===null!=(t.receiverClassId===null)||t.receiverRenderId!==null&&g.get(t.receiverRenderId)!==t.receiverClassId)return semantic(pointer(d,"receiverRenderId"),`graphs[${n}] fill receiver and class are mismatched.`);if(t.sourceInvocationId!==null&&!I.get("nestedComponents").has(t.sourceInvocationId))return semantic(pointer(d,"sourceInvocationId"),`graphs[${n}] fill has an unknown sourceInvocation.`);const f=t.locationId===null?void 0:b.get(t.locationId),u=t.fallbackLocationId===null?void 0:b.get(t.fallbackLocationId);if(t.policy==="template"){if(t.ownerRenderId===null||t.receiverRenderId===null||!new Set(["implicit","named","fallback"]).has(t.kind))return semantic(pointer(d,"policy"),`graphs[${n}] template fill ownership is inconsistent.`)}else if(t.policy==="python-detached"){if(t.kind!=="python"||t.ownerRenderId!==null||t.receiverRenderId===null||t.sourceInvocationId!==null||t.fallbackLocationId!==null)return semantic(pointer(d,"policy"),`graphs[${n}] detached Python fill ownership is inconsistent.`)}else if(t.kind!=="typed-default"||t.ownerRenderId!==null||t.receiverRenderId===null||t.sourceInvocationId!==null||t.fallbackLocationId!==null)return semantic(pointer(d,"policy"),`graphs[${n}] detached typed-default fill ownership is inconsistent.`);if(i){for(const[S,D]of[["locationId",t.locationId],["fallbackLocationId",t.fallbackLocationId]])if(D!==null&&!I.get("sourceLocations").has(D))return semantic(pointer(d,S),`graphs[${n}] fill has an unknown ${S}.`);const h=t.locationId===null?void 0:w.get(t.locationId),T=t.fallbackLocationId===null?void 0:w.get(t.fallbackLocationId);if(t.ownerRenderId===null!=(h===void 0))return semantic(pointer(d,"locationId"),`graphs[${n}] fill owner and source location are inconsistent.`);if(h&&(h[0]!==t.ownerRenderId||h[1]!==t.ownerClassId))return semantic(pointer(d,"locationId"),`graphs[${n}] fill source location owner is mismatched.`);if(T&&(T[0]!==t.receiverRenderId||T[1]!==t.receiverClassId))return semantic(pointer(d,"fallbackLocationId"),`graphs[${n}] fill fallback location receiver is mismatched.`)}if(t.policy==="template"){if(i){const h={implicit:"implicit-fill",named:"named-fill",fallback:"fallback-fill"}[t.kind];if(f!==h)return semantic(pointer(d,"locationId"),`graphs[${n}] template fill source location kind is mismatched.`);if(t.kind==="fallback"&&(t.fallbackLocationId===null||u!=="slot-outlet"))return semantic(pointer(d,"fallbackLocationId"),`graphs[${n}] fallback location kind is mismatched.`);if(t.kind!=="fallback"&&t.fallbackLocationId!==null)return semantic(pointer(d,"fallbackLocationId"),`graphs[${n}] supplied fill carrier is inconsistent.`)}if(t.kind==="fallback"){if(t.sourceInvocationId!==null)return semantic(pointer(d,"sourceInvocationId"),`graphs[${n}] fallback fill carrier is inconsistent.`)}else{if(t.sourceInvocationId===null)return semantic(pointer(d,"sourceInvocationId"),`graphs[${n}] supplied fill carrier is inconsistent.`);if(m.get(t.sourceInvocationId)?.[0]!==t.ownerRenderId)return semantic(pointer(d,"sourceInvocationId"),`graphs[${n}] supplied fill source invocation owner is mismatched.`)}}$.set(t.fillId,[t.ownerRenderId,t.receiverRenderId,t.locationId])}const k=new Map;for(let a=0;a<s.slotRegions.length;a+=1){const t=s.slotRegions[a],d=pointer(pointer(o,"slotRegions"),a);if(!I.get("fills").has(t.fillId))return semantic(pointer(d,"fillId"),`graphs[${n}] slot region has an unknown fill.`);if(t.parentRegionId!==null&&!I.get("slotRegions").has(t.parentRegionId))return semantic(pointer(d,"parentRegionId"),`graphs[${n}] slot region has an unknown parent.`);for(const[u,h]of Object.entries({receiverRenderId:t.receiverRenderId,ownerRenderId:t.ownerRenderId,transitionFromRenderId:t.transitionFromRenderId,resultOwnerRenderId:t.resultOwnerRenderId}))if(h!==null&&!p.has(h))return semantic(pointer(d,u),`graphs[${n}] slot region.${u} is unknown.`);if(i){for(const[h,T]of[["slotLocationId",t.slotLocationId],["sourceLocationId",t.sourceLocationId]])if(T!==null&&!I.get("sourceLocations").has(T))return semantic(pointer(d,h),`graphs[${n}] slot region has an unknown ${h}.`);const u=t.slotLocationId===null?void 0:w.get(t.slotLocationId);if(u&&u[0]!==t.receiverRenderId)return semantic(pointer(d,"slotLocationId"),`graphs[${n}] slot region slot location receiver is mismatched.`);if(t.slotLocationId!==null&&b.get(t.slotLocationId)!=="slot-outlet")return semantic(pointer(d,"slotLocationId"),`graphs[${n}] slot region slot location kind is mismatched.`)}const f=$.get(t.fillId);if(!f||f[0]!==t.ownerRenderId||f[1]!==t.receiverRenderId||f[2]!==t.sourceLocationId)return semantic(pointer(d,"fillId"),`graphs[${n}] slot region ownership does not match its fill.`);k.set(t.regionId,[t.ownerRenderId,t.receiverRenderId,t.parentRegionId,t.transitionFromRenderId])}if(cycle(new Map(Array.from(k,([a,t])=>[a,t[2]]))))return semantic(pointer(o,"slotRegions"),`graphs[${n}] slot region ancestry contains a cycle.`);for(let a=0;a<s.slotRegions.length;a+=1){const t=s.slotRegions[a],d=t.parentRegionId===null?t.receiverRenderId:k.get(t.parentRegionId)?.[0]??null;if(t.transitionFromRenderId!==d)return semantic(pointer(pointer(pointer(o,"slotRegions"),a),"transitionFromRenderId"),`graphs[${n}] slot region scope transition does not match its ancestry.`)}const O=new Map;for(let a=0;a<s.componentExecutionOrderConstraints.length;a+=1){const t=s.componentExecutionOrderConstraints[a],d=pointer(pointer(o,"componentExecutionOrderConstraints"),a),f=m.get(t.invocationId);if(!f||f[0]!==t.parentRenderId||f[1]!==t.childRenderId)return semantic(pointer(d,"invocationId"),`graphs[${n}] component execution order constraint does not match its invocation.`);const u=O.get(t.parentRenderId)??[];u.push(t.childRenderId),O.set(t.parentRenderId,u)}if(executionCycle(O))return semantic(pointer(o,"componentExecutionOrderConstraints"),`graphs[${n}] component execution order contains a cycle.`);if(cycle(L))return semantic(pointer(o,"componentInstances"),`graphs[${n}] logical instance ancestry contains a cycle.`)}return null},REVISION=/^[0-9a-f]{64}$/,MANIFEST_FIELDS=new Set(["protocol","revision","mode","graphs","delimiters"]),REQUIRED_MANIFEST_FIELDS=["protocol","revision","mode","graphs","delimiters"],DELIMITER_FIELDS=new Set(["format"]),validateManifestShape=(r,e)=>{const i=validateStrictJson(r,e);if(i)return i;if(!isPlainObject(r))return{path:e,category:"type",message:"The client-graph manifest must be an object."};for(const o of REQUIRED_MANIFEST_FIELDS)if(!hasOwn(r,o))return{path:pointer(e,o),category:"required",message:`The manifest requires '${o}'.`};const c=firstUnknown(r,MANIFEST_FIELDS);if(c!==null)return{path:pointer(e,c),category:"unknown_field",message:"The manifest has an unknown field."};if(r.protocol!==PROTOCOL)return{path:pointer(e,"protocol"),category:typeof r.protocol=="string"?"enum":"type",message:`The manifest protocol must be ${PROTOCOL}.`};if(typeof r.revision!="string")return{path:pointer(e,"revision"),category:"type",message:"The manifest revision must be a string."};if(!REVISION.test(r.revision))return{path:pointer(e,"revision"),category:"pattern",message:"The manifest revision must be lowercase SHA-256."};if(typeof r.mode!="string")return{path:pointer(e,"mode"),category:"type",message:"The manifest mode must be a string."};if(r.mode!=="production"&&r.mode!=="development")return{path:pointer(e,"mode"),category:"enum",message:"The manifest mode must be production or development."};if(!Array.isArray(r.graphs))return{path:pointer(e,"graphs"),category:"type",message:"The manifest graphs must be an array."};for(let o=0;o<r.graphs.length;o+=1){const l=validateGraph(r.graphs[o],pointer(pointer(e,"graphs"),o),!1);if(l)return l}const n=pointer(e,"delimiters");if(!isPlainObject(r.delimiters))return{path:n,category:"type",message:"The manifest delimiters must be an object."};if(!hasOwn(r.delimiters,"format"))return{path:pointer(n,"format"),category:"required",message:"The manifest delimiters require 'format'."};const s=firstUnknown(r.delimiters,DELIMITER_FIELDS);return s!==null?{path:pointer(n,s),category:"unknown_field",message:"The manifest delimiters have an unknown field."}:r.delimiters.format!==OWNERSHIP_COMMENT_PREFIX?{path:pointer(n,"format"),category:typeof r.delimiters.format=="string"?"enum":"type",message:`The ownership-comment prefix must be ${OWNERSHIP_COMMENT_PREFIX}.`}:null},validateRevision=(r,e="")=>!isPlainObject(r)||typeof r.revision!="string"||!REVISION.test(r.revision)||revisionForManifest(r)===r.revision?null:{path:pointer(e,"revision"),category:"correlation",message:"The revision does not match the canonical unsigned manifest."},validateManifest=(r,e="")=>{const i=validateManifestShape(r,e);if(i)return i;const c=validateRevision(r,e);return c||validateRelationships(r,e)},assertValidManifest=r=>{const e=validateManifest(r);if(e)throw new ProtocolValueError(e);return r},CitryClientGraphProtocol={OWNERSHIP_COMMENT_PREFIX,ownershipRevisionAlias,parseOwnershipComment,ProtocolValueError,assertValidManifest};
  /*</citry-client-graph-v1>*/
  ...
  // The wire comments use the first eight hex characters as a local alias so
  // rendered HTML remains readable. Full revisions remain the identity for
  // every graph/state map. One live or provisional graph reserves an alias;
  // a different full revision with the same alias is rejected before it can
  // publish any state.
  var ownershipRevisionsByAlias = new Map();
  ...
  var ownershipRevisionAlias = function (revision) {
    return CitryClientGraphProtocol.ownershipRevisionAlias(revision);
  };

  var assertOwnershipRevisionAliasAvailable = function (revision) {
    var alias = ownershipRevisionAlias(revision);
    var reservedRevision = ownershipRevisionsByAlias.get(alias);
    if (reservedRevision !== undefined && reservedRevision !== revision) {
      throw new TypeError(
        "[Citry] graph: revision alias " + alias +
          " is already in use by a different graph revision."
      );
    }
    return alias;
  };

  var reserveOwnershipRevisionAlias = function (revision) {
    var alias = assertOwnershipRevisionAliasAvailable(revision);
    ownershipRevisionsByAlias.set(alias, revision);
    return alias;
  };

  var releaseOwnershipRevisionAlias = function (revision) {
    var alias = ownershipRevisionAlias(revision);
    if (ownershipRevisionsByAlias.get(alias) === revision) {
      ownershipRevisionsByAlias.delete(alias);
    }
  };

  var fullOwnershipRevision = function (alias) {
    return ownershipRevisionsByAlias.get(alias) || null;
  };
  ...
  var validatePhysicalCaps = function (revision, expected, root) {
    root = root || document;
    var revisionAlias = ownershipRevisionAlias(revision);
    var revisionPrefix = OWNERSHIP_COMMENT_PREFIX + ":" + revisionAlias + ":";
    ...
    while ((node = comments.nextNode())) {
      ...
      if (ownership.revisionAlias !== revisionAlias) continue;
  ...
  var stageOwnershipManifest = function (manifest, capRoot) {
    ...
    assertOwnershipRevisionAliasAvailable(validated.revision);
  ...
  var rangeCapInfo = function (node) {
    ...
    if (ownership) {
      var revision = fullOwnershipRevision(ownership.revisionAlias);
      if (revision === null) {
        throw new TypeError(
          "[Citry] range morph received ownership caps for unknown revision alias " +
    ...
    var match = /^citry:p1:([0-9a-f]{8}):([A-Za-z0-9_-]+):([0-9]+):([ir]):([0-9]+):([se])$/.exec(node.data.trim());
    if (!match) return null;
    var runtimeRevision = fullOwnershipRevision(match[1]);
    if (runtimeRevision === null) {
      throw new TypeError(
        "[Citry] range morph received runtime caps for unknown revision alias " + match[1] + "."
      );
    }
    return {
  ...
  var validateRuntimePlacementCaps = function (revision, expected) {
    var revisionAlias = ownershipRevisionAlias(revision);
    var prefix = "citry:p1:" + revisionAlias + ":";
    ...
    while ((node = comments.nextNode())) {
      ...
      var match = /^citry:p1:([0-9a-f]{8}):([A-Za-z0-9_-]+):([0-9]+):([ir]):([0-9]+):([se])$/.exec(text);
      if (!match || match[1] !== revisionAlias) {
  ...
  var applyAdoptionTransfers = function (state) {
    var revisionAlias = ownershipRevisionAlias(state.publicRevision.revision);
    state.adoption.transfers.forEach(function (candidates, key) {
      ...
      live.forEach(function (physical, index) {
        ...
        var prefix = placementId == null
          ? OWNERSHIP_COMMENT_PREFIX + ":" + revisionAlias + ":"
          : "citry:p1:" + revisionAlias + ":" + placementId + ":";
  ...
  var removeExcludedAdoptionCaps = function (state, expected) {
    ...
    var revisionAlias = ownershipRevisionAlias(revision);
    var canonicalPrefix = OWNERSHIP_COMMENT_PREFIX + ":" + revisionAlias + ":";
    var runtimePrefix = "citry:p1:" + revisionAlias + ":";
    var comments = document.createTreeWalker(document, NodeFilter.SHOW_COMMENT);
    var excluded = [];
    for (var node = comments.nextNode(); node; node = comments.nextNode()) {
      ...
      var key = null;
      var ownership = CitryClientGraphProtocol.parseOwnershipComment(text);
      if (ownership && ownership.revisionAlias === revisionAlias) {
        ...
        var runtime = /^citry:p1:([0-9a-f]{8}):([A-Za-z0-9_-]+):([0-9]+):([ir]):([0-9]+):([se])$/.exec(text);
        if (!runtime || runtime[1] !== revisionAlias) {
  ...
  var prepareOwnershipAdoption = function (manifest, capRoot) {
    ...
    reserveOwnershipRevisionAlias(staged.revision);
  ...
  var pruneInactiveOwnershipRevisions = function () {
    ...
    ownershipStates.forEach(function (state, revision) {
      ...
      releaseOwnershipRevisionAlias(revision);
  ...
  var abortOwnershipAdoption = function (transaction, error) {
    ...
    var revisionAlias = ownershipRevisionAlias(transaction.revision);
    var canonicalPrefix = OWNERSHIP_COMMENT_PREFIX + ":" + revisionAlias + ":";
    var runtimePrefix = "citry:p1:" + revisionAlias + ":";
    var comments = document.createTreeWalker(document, NodeFilter.SHOW_COMMENT);
    var ownedComments = [];
    ...
    releaseOwnershipRevisionAlias(transaction.revision);

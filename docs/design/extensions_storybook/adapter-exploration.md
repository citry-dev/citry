# Storybook adapter exploration with Citry UI

**Status (2026-07-29): historical spike evidence for the optional Storybook
extension; no adapter selected.** Both candidate adapters can preview the
current server-static Citry UI scenarios and the private reactive readiness probe,
...
remaining interactive browser-readiness scenarios remain prerequisites for
selecting an adapter, but do not gate Citry UI development or publication.

This report implements the first half of the optional projection described in
[`../extensions_storybook.md`](../extensions_storybook.md) over the
[`Citry UI scenario catalog`](../ui_research/scenario-catalog.md). The
disposable implementation is in
[`packages/py/citry_ui/storybook`](../../../packages/py/citry_ui/storybook/).

## 1. Scope and role

Storybook is evaluated as an optional maintainer previewer. It browses isolated
...
performance work. Citry's docs playground and live components now provide the
first-party, Node-free documentation preview surface.
...
## 8. Provisional conclusion and deferred extension work

Both candidates remain viable for an optional extension. No adapter is
selected from the static cases or the first reactive probe.
...
If the Storybook extension becomes active work, its next evidence sequence is:
...
that combined evidence is complete. Citry UI's production slice proceeds
independently through its specifications, live documentation, and direct
quality suite.

...
| 5b.11 | `version-picker-component` | Header dropdown reads versions.json | M | ✓ | main §4.6, 11.7 §5, 11.11 §4.1 | **done** | `VersionPicker` component (replaces the static badge); behavior in `site.js`, base-path-agnostic `/v/<version>/` derivation. Browser round-trip verified. Switches to version home (preserve-page is a Phase-7 enhancement). **Later:** also populates on root (non-`/v/`) pages - notably the current docs at the site root - by falling back to the build-emitted `<meta name="djc-base-path">` (was `/v/`-only) |
| 5b.12 | `docs-versions-yaml` | Top-level YAML config | S | | 11.7 §3.2 | **done** | `docs_site/docs_versions.yml` at the docs-project root. Strictly loaded by `bootstrap.load_versions_config`; keys cover tag pattern/include/exclude/bounds, latest alias, publish window, and indexing retention. `oldest=0.1.0`, Citry's first release. |
...
| **3 — docs-build-all** | `docs_versions.yml`, bootstrap core, worktree walker | 5b.8, 5b.9, 5b.12 |
...
| 5c.2 | `robots-txt` | `robots.txt` (disallow old versions; wildcard crawler policy; sitemap pointer) | S | | 11.12 §2.A.3 | **done** (Chunk 2) | `_internal/seo.py::write_robots`; one `User-agent: *` group allows all crawlers and adds generated `Disallow: /v/<version>/` rules for every version except the two newest releases plus `latest`, followed by the sitemap pointer. Search and AI crawlers share that policy without a named-bot list. Uses the actual `/v/<version>/` scheme, not the spike's `/v0.x/`. |
...
| 5c.10 | `llms-txt` | `/llms.txt` short index + `/llms-full.txt` concatenation | M | | 11.12 §3.B.1 | **done** (Chunk 3) | `_internal/llms.py`; `llms.txt` follows the v2 H2 file-list shape and links directly to emitted page-level `index.md` companions. Current pages advertise their companion with `rel="alternate"` and the covering `/llms.txt` with `rel="describedby"`. `llms-full.txt` remains available as a documented nonstandard bulk export and includes the home page. Root-build only (like SEO); `--no-llms` opt-out. Authored companions contain expanded Markdown; generated Reference companions may retain HTML-rich reference fragments without site chrome. **Two source fixes folded in:** (1) `pipeline.expand_snippets` resolves `--8<--` file includes in companions and the bulk export; (2) the description extractor skips raw HTML, comments, and include directives, drops images and badge links, resolves cross-references, and preserves `snake_case`. |
...
| **2: Crawl & index files** | `sitemap.xml`, `robots.txt`, and `indexing.json` are post-build root emitters sharing `_internal/seo.py` and the versions manifest. Generated by the current-version root build; version snapshots and canonical-to-current alignment are handled by the versioning and assembly stages. `--no-seo` opt-out. | 5c.1, 5c.2, 5c.11 | **done** |
| **3: llms.txt** | v2 `/llms.txt` links to emitted page companions; `/llms-full.txt` remains a nonstandard bulk export; page heads expose companion `alternate` plus `/llms.txt` `describedby`; descriptions and labels are normalized to parser-safe single lines | 5c.10 | **done** |
...
| # | ID | Name | Effort | Critical | Source | Status | Notes |
|---|---|---|---|---|---|---|---|
| 6.1 | `materialize-redirects-script` | Convert `latest/` symlink → redirect HTML files | S | ✓ | 11.8 §6, §7 | **done** | Folded into the gh-pages import (6.2): `docs_import_ghpages` re-materializes `latest` via `versioning.materialize_alias` (redirect stubs, no symlink). The imported tree has no symlinks at all - Windows-safe |
| 6.2 | `import-gh-pages-tree` | One-time mirror of `origin/gh-pages` into `docs_site/versions/` (57 versions + dev) | M | ✓ | 11.8 §5.1, §7 | **done** | New `docs_import_ghpages` command: extracts each version subtree via `git archive`, stamps a synthesized `_build_info.json` (release-tag commit + `imported-ghpages` builder_version sentinel), copies the manifest, re-materializes `latest`. Imported 59 versions + dev (live branch count, the spike's "57" was stale). Target `docs_site/versions/` per main §4.0a |
| 6.3 | `docs-build-check-validation` | Validate imported tree before commit | S | ✓ | 11.8 §5.1, §7 | **done** | `docs_versions_check` passes on all 59 imports + `latest`. Added frozen-import awareness (`versioning.is_frozen_import`): `cross_version_link` skips frozen imports (historical HTML we never rebuild - old mkdocs theme templates, long-dead links); `versions_manifest` no longer requires a root `index.html` for them (0.111 shipped without one). New-builder versions stay fully checked; +3 guard tests |
| 6.4 | `ci-deploy-site-via-actions` | Rewrite docs CI: build `docs_site` → `site/` (current + `docs_site/versions/*`), deploy `site/` via GitHub Actions | M | ✓ | main §4.0a, §4.6 | **done** | New `docs_assemble` command builds the current version into the site root + mounts every committed snapshot under `/v/*` (verified: 59 versions + `latest` + `versions.json` + `/benchmarks/` passthrough; robots.txt disallows 57 old `/v/*`). `release-docs.yml` assembles + deploys via `configure-pages`/`upload-pages-artifact`/`deploy-pages` with a `docs-build-deploy` concurrency group. **Manual at real cutover:** flip the repo Pages source to "GitHub Actions". Supersedes "switch Pages source to master/docs/v/". **Static-assembly piece already built:** `builder.collect_static()` copies `/static` into the output via the staticfiles *finders*. `build_docs` runs it **by default** for the current-version build (`--no-collectstatic` to skip); it's skipped for per-version snapshots so they don't each duplicate static (the deploy mounts one shared `/static` at the root). The Lighthouse workflow (5c.13) uses the same assembly. Note: must copy from finders, not override `settings.STATIC_ROOT` + `collectstatic` (the storage caches its dest mid-build). **Post-design additions (from the fork-preview deploy):** (1) a **publish window** (`docs_versions.yml` → `publish.window`, default 15) bounds how many of the newest versions actually deploy, to stay under GitHub Pages' 1 GB cap - the committed tree always keeps all, `docs_assemble` mounts + serves a trimmed `versions.json` for the subset; (2) **base-path support** (`settings.SITE_BASE_PATH` / `DOCS_BASE_PATH` + a post-build `apply_base_path` URL rewrite + base-path-aware `search.js` / version picker) lets the site serve under a project-Pages subpath (`/<repo>/`), not just a domain root |
| 6.5 | `gh-pages-branch-deletion` | Delete `gh-pages` branch (deferred 3-6 months) | S | | 11.8 §5.1, §8.2 | pending | Cleanup, not blocker |
| 6.6 | `cutover-docs-cleanup` | Delete `docs_old/` + `mkdocs.yml`; remove mkdocs/material/mike deps from pyproject.toml; `grep -rn docs_old .` and update every straggler | M | ✓ | main §4.0a, §6 | **done** | Deleted `docs_old/` + `mkdocs.yml`; removed the 16 mkdocs/material/mike/mkdocstrings deps (kept survivors: pymdown-extensions, griffe, lxml, verspec, pagefind, minify-html, pygments-djc). Builder verified self-contained (149 pages build with those uninstalled). Swept all `docs_old` + `master/docs/` stragglers. The `docs_old` rename makes this a search. **Also grep `master/docs/`**: absolute GitHub URLs (README image, CHANGELOG link, mkdocs `edit_uri`) still say `docs/` and won't be caught by the `docs_old` grep |
...
| 6.8 | `examples-move` | Move `docs_old/examples/` → `docs_site/examples/`; update pytest `testpaths`, ruff override, sampleproject `EXAMPLES_DIR` | M | ✓ | main §4.0a, §4.1 | **done** | Moved; repointed both `EXAMPLES_DIR`s, testpaths, ruff override, `--8<--` snippet includes; excluded from mypy (same-named demo modules). Verified: 9 examples discovered, 28 tests collected. Was Phase-1 feature 1.4 |
| 6.9 | `devguides-move` | Move `docs_old/community/devguides/` → internal `docs/devguides/` (was never meant to be user-facing) | S | | main §4.0a | **done** | Moved 3 articles; dropped the mkdocs `.nav.yml` files |
| 6.10 | `devguides-relevance-review` | Review each devguide article for whether it's still relevant/accurate before keeping it as internal docs | S | | main §4.0a | **done** | All 3 relevant. Fixed `dependency_mgmt.md` (4-field `_RENDERED` marker, real `{% component_js/css_dependencies %}` names, dead cross-link) + a `{% endfill %}` typo in `slot_rendering.md` |
| 6.11 | `benchmark-report-relocation` | Relocate asv report `docs_old/benchmarks/` → `benchmarks/report/`; add static-passthrough copy into the build; update `asv.conf.json` `html_dir`, `release-docs.yml`, `benchmarks/README.md` | M | | main §4.0a | **done** | Moved; updated `asv.conf.json html_dir`, `release-docs.yml`, `benchmarks/README.md`. Added `settings.STATIC_PASSTHROUGHS` + `builder.copy_passthroughs` (explicit `(source -> mount)` list), wired into `build_docs`/`docs_assemble`; serves at `/benchmarks/` (57 files). Static passthrough, not "serve index.html if present" |
| 6.12 | `canonical-latest-alignment` | Implement the §2.A.1 canonical strategy properly: pages canonical to their `/latest/` (root) counterpart, not self-referential versioned URLs; pages absent from latest canonical to self + `noindex,follow`; `/latest/` (root) canonical to itself | M | ✓ | 11.12 §2.A.1 | **done (part a 5c / part b Phase 6)** | **(a) DONE in 5c:** current-version build → root canonical, realigning og:url / breadcrumb / sitemap / indexing.json. **(b) Phase 6:** the old `/v/<ver>/` snapshots are now **frozen gh-pages imports** - we never rewrite their HTML, so their baked canonicals stay as-is. The achievable intent (keep old versions out of search) is met by `seo.write_robots`, which - now that the imported manifest exists - disallows every `/v/<version>/` except the newest few + `latest` (verified: 57 disallows in the assembled robots.txt). The frozen HTML's stale canonicals are harmless under that disallow |
...
| 10.4 | `cve-bundle-audit` | CVE audit on frozen Material/plugin bundles | M | | 11.8 §8.1 | **done** | Sweep complete - see [`cve_audit_10.4.md`](cve_audit_10.4.md). **Verdict: accept residual risk, no rebuild triggered.** Live docs are jQuery/Material-free (new builder). Carriers: the asv report's CDN jQuery 3.3.1 + Bootstrap 3.1.1 (EOL XSS CVEs, but only static first-party data, no URL->DOM-sink path found) and one frozen version `/v/0.92/` (Material 9.5.16, search RXSS <9.5.32; all 58 others 9.5.34+). timeago 4.0.2 / lunr / pyodide clean. Follow-ups (SRI + jQuery bump on the asv report; 0.92 rebuild via 10.1) tracked, not blocking |
| 10.5 | `docs-versions-yaml-per-version-freeze-flag` | Per-version `freeze=true` schema extension | S | | 11.8 §9 | pending | Only if needed |
...
| 5a | Search v1 | 6 | 4 | **6/6 done** (5a.1-5a.6 all shipped: pagefind index, search modal + header trigger, empty/no-results/error states, `/` `⌘K` `?q=` `?h=` shortcuts, custom 404) |
...
| 6 | Cutover | 12 | 8 | **11/12 done** - cutover implemented on the branch (not yet merged/deployed). 6.1/6.2/6.3/6.4/6.6/6.8/6.9/6.10/6.11/6.12 done; 6.7 folded into 3b.25. Only 6.5 (`gh-pages-branch-deletion`) deferred by design (post-cutover cleanup, 3-6 months). Manual at real cutover: flip the repo Pages source to "GitHub Actions" |
...
| 10+ | Deferred / post-launch maintenance | 7 | 0 | **2/7 done** (10.4 `cve-bundle-audit`: swept, residual risk accepted; 10.7 `dev-deploy-ci-flow`: dev builds + deploys, never committed); rest pending |
| **Total** | | **226** | **112** | **210/226 done** (phases 0-4, 5a, 5b, 5c, 5d complete; Phase 6 cutover 11/12 done on-branch; 10.4 + 10.7 done. Remaining: 6.5 deferred, phases 7-9 + the rest of 10+ pending; 5c.15 dropped; 1.26 → 6.12) |

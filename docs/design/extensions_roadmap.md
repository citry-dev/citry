...
| **Storybook** | An `ExtensionCommand` consumes an explicit Python scenario catalog and emits a deterministic preview projection; scenario routes return server-rendered Citry output. | The Phase 7 entry gate first compares server-static scenarios on Server/Webpack and HTML/Vite, then previews interactive readiness cases through both before selection. Playwright runs behavioral journeys against standalone routes. A later generic extension builds on `ExtensionCommand`, `Extension.urls`, component introspection, and the proven scenario contract instead of requiring every component to declare nested Storybook metadata. |

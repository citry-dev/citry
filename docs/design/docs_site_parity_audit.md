...
| 5b.12 | done | docs-versions-toml | Done (workstream D): `docs_site/docs_versions.toml` (pattern/include/exclude/oldest/newest, [aliases] latest, [publish] window) parsed by `bootstrap.load_versions_config` (stdlib tomllib) and read by `build-all`; `publish_window` now sources from it (single source of truth). |
...
| 10.5 | n/a | docs-versions-toml-per-version-freeze-flag | ✓ n/a (latent) | N/A: the freeze flag exists to mark imported/frozen gh-pages versions, which citry has none of. Reference marks it 'Only if needed'. |

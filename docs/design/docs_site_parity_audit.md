...
| 1.23 | done | docpage-head-block (unified head: title, description, canonical, viewport, favicon, robots, alternate...) | ✓ | Ported. Current pages identify their page-level `index.md` companion with `rel=alternate` and the covering `/llms.txt` with `rel=describedby`; theme-color is not explicitly emitted (minor vs the inventory's field list). |
...
| 5c.2 | done | robots-txt | ✓ done (multi-version) | Done (workstream D): `write_robots` emits one wildcard group with `Disallow: /v/<v>/` for every version outside the newest two plus `latest` (read from the committed versions.json), followed by the sitemap. The wildcard policy covers search and AI crawlers without a named-bot list. |
...
| 5c.10 | done | llms-txt | ✓ | `llms.txt` follows the v2 file-list shape and links to emitted page-level Markdown companions. `llms-full.txt` remains as a documented nonstandard bulk export. Page `alternate` and `describedby` relations are projected through version and deployment base paths. |

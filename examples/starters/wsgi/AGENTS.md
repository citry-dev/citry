# Agent instructions for the Citry bare WSGI starter
...
- Start with `app/main.py` for the dispatcher, `app/citry_app.py`
  for Citry setup, `app/components/` for components, and `app/data.py` for data.
- Initialize the shared Citry instance before server threads start. Preserve
  `SCRIPT_NAME` and `PATH_INFO` handling when dispatching `/citry` requests.

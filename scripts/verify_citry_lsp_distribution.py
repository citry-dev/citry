...
assert citry_lsp.MINIMUM_CITRY_VERSION <= citry_version, citry_version
assert citry_lsp.SERVER_VERSION == "__VERSION__"
assert citry_lsp.PROTOCOL_VERSION == 1
assert citry_lsp.MINIMUM_CITRY_SERIES == citry_lsp.MINIMUM_CITRY_VERSION[:2]
...
def smoke_wheel(wheel: Path, *, cwd: Path, dependency_wheels: Sequence[Path] = ()) -> None:
    """Install one wheel with its selected or public dependencies and start the server."""

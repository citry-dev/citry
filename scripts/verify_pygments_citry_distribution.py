"""Build, inspect, install, and promote the pygments-citry release pair."""
...
PACKAGE_ROOT: Final = REPO_ROOT / "packages" / "py" / "pygments_citry"
SOURCE_ROOT: Final = PACKAGE_ROOT / "pygments_citry"
MAX_WHEEL_BYTES: Final = 128 * 1024
MAX_SDIST_BYTES: Final = 256 * 1024
...
def package_version() -> str:
    ...
    if not isinstance(version, str) or not version:
        raise DistributionVerificationError("pygments-citry has no project version")
    return version


def expected_release_filenames(version: str) -> set[str]:
    """Return the complete, closed pygments-citry release set."""
    return {
        f"pygments_citry-{version}-py3-none-any.whl",
        f"pygments_citry-{version}.tar.gz",
    }
...
def _require_metadata(metadata: Message, *, artifact: Path, version: str) -> None:
    if metadata.get("Name", "").lower().replace("_", "-") != "pygments-citry":
...
def sdist_inventory(path: Path) -> dict[str, str]:
    ...
    with tarfile.open(path, "r:gz") as archive:
        for member in archive.getmembers():
            ...
            inventory[member.name] = sha256_bytes(stream.read())
...
def verify_wheel(path: Path, *, version: str) -> dict[str, Any]:
    ...
    expected_name = f"pygments_citry-{version}-py3-none-any.whl"
    ...
    dist_info = f"pygments_citry-{version}.dist-info/"
    try:
        with zipfile.ZipFile(path) as archive:
            ...
            expected_members = {f"pygments_citry/{name}" for name in source_inventory()} | controls
            ...
            if archive.read(f"{dist_info}entry_points.txt") != (
                ...
                raise DistributionVerificationError(f"{path.name} has unexpected Pygments entry points")
            if archive.read(f"{dist_info}top_level.txt") != b"pygments_citry\n":
            ...
            actual_payload = {
                name.removeprefix("pygments_citry/"): sha256_bytes(archive.read(name))
                for name in names
                if name.startswith("pygments_citry/")
            }
...
def _checkout_sdist_files() -> dict[str, bytes]:
    ...
    # Setuptools includes top-level test modules by default. Keep that boundary
    # explicit so an unnoticed generated or executable file fails qualification.
...
def verify_sdist(path: Path, *, version: str) -> dict[str, Any]:
    ...
    expected_name = f"pygments_citry-{version}.tar.gz"
    if path.name != expected_name:
    ...
    expected_root = f"pygments_citry-{version}"
    try:
        with tarfile.open(path, "r:gz") as archive:
    ...
    generated = {
        "PKG-INFO",
        "setup.cfg",
        "pygments_citry.egg-info/PKG-INFO",
        "pygments_citry.egg-info/SOURCES.txt",
        "pygments_citry.egg-info/dependency_links.txt",
        "pygments_citry.egg-info/entry_points.txt",
        "pygments_citry.egg-info/requires.txt",
        "pygments_citry.egg-info/top_level.txt",
    }
    ...
    if relative["pygments_citry.egg-info/PKG-INFO"] != relative["PKG-INFO"]:
    ...
    if relative["pygments_citry.egg-info/dependency_links.txt"] != b"\n":
        raise DistributionVerificationError(f"{path.name} contains unexpected dependency links")
    if relative["pygments_citry.egg-info/entry_points.txt"] != (
        b"[pygments.lexers]\n"
        b"citry = pygments_citry.lexers:CitryPythonLexer\n"
        b"citry-html = pygments_citry.citry_html:CitryHtmlLexer\n"
    ):
        raise DistributionVerificationError(f"{path.name} contains unexpected Pygments entry points")
    if relative["pygments_citry.egg-info/requires.txt"] != b"fluent.pygments==1.0\npygments>=2.20\n":
        raise DistributionVerificationError(f"{path.name} contains unexpected runtime dependencies")
    if relative["pygments_citry.egg-info/top_level.txt"] != b"pygments_citry\n":
        raise DistributionVerificationError(f"{path.name} contains an unexpected top-level package")
    sources = relative["pygments_citry.egg-info/SOURCES.txt"].decode("utf-8").splitlines()
    expected_sources = set(checkout) | {name for name in generated if name.startswith("pygments_citry.egg-info/")}
...
def verify_staged_bundle(bundle_dir: Path) -> dict[str, Any]:
    ...
    wheel = by_name[f"pygments_citry-{version}-py3-none-any.whl"]
    sdist = by_name[f"pygments_citry-{version}.tar.gz"]
...
def _build_sdist_wheel(sdist: Path, directory: Path, *, cwd: Path) -> Path:
    # Resolve the artifact before the subprocess moves outside the checkout.
    sdist = sdist.resolve()
...
citry = get_lexer_by_name("citry")
citry_html = get_lexer_by_name("citry-html")
fluent = get_lexer_by_name("fluent")
ftl = get_lexer_by_name("ftl")

import pygments_citry

assert importlib.metadata.version("pygments-citry") == "__VERSION__"
assert pathlib.Path(pygments_citry.__file__).resolve().is_relative_to(pathlib.Path(sys.prefix).resolve())
assert citry.name == "Citry Python"
assert citry_html.name == "Citry HTML"
...
component = (
    ...
    '    template = """\n'
    '      <p $c-tr:greeting[title]="{ name: user.name }">{{ name }}</p>\n'
    '    """\n'
    '    messages = """\n'
    '      # Translator note\n'
    '      greeting = Hello, { $name }.\n'
...
tokens = [(token, value) for token, value in citry.get_tokens(component) if value.strip()]
assert (Name.Attribute, "$c-tr:greeting[title]") in tokens
assert (Name.Other, "user") in tokens
assert (Name.Constant, "greeting") in tokens
assert (Name.Variable, "$name") in tokens
assert (Comment.Multiline, "# Translator note") in tokens

template_tokens = [
    (token, value)
    for token, value in citry_html.get_tokens('<p c-if="visible">{{ name }}</p>')
    if value.strip()
]
assert (Name.Attribute, "c-if") in template_tokens
assert (Name, "visible") in template_tokens
assert (Name, "name") in template_tokens
'''
...
def smoke_wheel(wheel: Path, *, cwd: Path) -> None:
    """Install one wheel with public binary dependencies and exercise each lexer."""
    with tempfile.TemporaryDirectory(prefix="pygments-citry-install-") as temporary:
        root = Path(temporary)
...
def verify_artifacts(source_wheel: Path, sdist: Path, rebuilt_wheel: Path) -> dict[str, Any]:
    ...
    require_equal(
        ...
        source_files,
        package_payload(source_wheel_files, "pygments_citry"),
    )
    ...
    require_equal(
        ...
        source_files,
        package_payload(sdist_files, f"{sdist_root}/pygments_citry"),
    )
    return {
...
def verify_dist_directory(dist_dir: Path, *, smoke: bool) -> tuple[Path, Path, dict[str, Any]]:
    ...
    version = package_version()
    wheel = _unique(dist_dir, f"pygments_citry-{version}-py3-none-any.whl")
    sdist = _unique(dist_dir, f"pygments_citry-{version}.tar.gz")
    with tempfile.TemporaryDirectory(prefix="pygments-citry-sdist-") as temporary:
        root = Path(temporary)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        parser.exit(1, f"pygments-citry distribution verification failed: {error}\n")

---
title: Slots
url: https://citry.dev/v/0.4.0/concepts/slots/
...
[Add flexible content](/v/0.4.0/getting-started/add-slots/). This page explains the
full composition model and the choices you have when designing a component.

## Define the places another template can fill

Add the [`<c-slot>` built-in](/v/0.4.0/reference/builtins/#c-slot) wherever the
...
The inner [`Slots` class](/v/0.4.0/reference/component/#citry-component-slots) describes the names this
component accepts. [`SlotInput`](/v/0.4.0/reference/slots/#citry-slotinput) describes content for one
...
[`<c-fill>` tags](/v/0.4.0/reference/builtins/#c-fill) in the body:
...
[Understand slot scope](/v/0.4.0/concepts/client-interactivity/#understand-slot-scope)
...
The value is an immutable [`SlotData`](/v/0.4.0/reference/slots/#citry-slotdata) record. Identifier keys
...
grammar and errors, see [`<c-fill>`](/v/0.4.0/reference/builtins/#c-fill).
...
[`Slot`](/v/0.4.0/reference/slots/#citry-slot). Ordinary strings are escaped. A `None` value means the
slot was not filled. See the [Slot Reference](/v/0.4.0/reference/slots/#citry-slot) for callable fills,
...
[`c-bind`](/v/0.4.0/syntax/dynamic-attributes/#c-bind-spread) for mapping evaluation
...
The `original` value is a [`Slot`](/v/0.4.0/reference/slots/#citry-slot), so inserting it renders the
slot's fallback at that point.

## Next steps

- [Provide and inject](/v/0.4.0/concepts/provide-and-inject/) passes data through a
  whole rendered subtree.
- [Client interactivity](/v/0.4.0/concepts/client-interactivity/) explains ownership
  when slots contain Alpine expressions.
- [Inputs and validation](/v/0.4.0/concepts/inputs-and-validation/) covers typed

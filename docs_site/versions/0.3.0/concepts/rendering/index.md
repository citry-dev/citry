---
title: Rendering
url: https://citry.dev/v/0.3.0/concepts/rendering/
...
Calling a component class does not render anything and does not build a component instance. It returns a [CitryElement](/v/0.3.0/reference/rendering/#citry-citryelement), a small record of "what to render": the class, the keyword arguments, and any slots.
...
Because a [CitryElement](/v/0.3.0/reference/rendering/#citry-citryelement) is just a description, you can render it on its own or pass it into another component and render it there. See [Build a page from components](/v/0.3.0/getting-started/build-page/) for how one component embeds another.

### Render

`render()` on the element runs the pipeline for the element and everything inside it, and returns a [CitryRender](/v/0.3.0/reference/rendering/#citry-citryrender). This is deliberately an object, not a string. Keeping it as an object means an already-rendered subtree can be reused, passed into another component, or embedded in a `{{ ... }}` expression, carrying its collected JS and CSS with it.
...
If you need a stable identity, hold on to the [CitryElement](/v/0.3.0/reference/rendering/#citry-citryelement) or the [CitryRender](/v/0.3.0/reference/rendering/#citry-citryrender), not something you read off one of them.
...
The rendering methods live on the element and the render, not on [Component](/v/0.3.0/reference/component/#citry-component):

- `render()` and `__str__` are on [CitryElement](/v/0.3.0/reference/rendering/#citry-citryelement).
- `serialize()`, `__str__`, and `__bytes__` are on [CitryRender](/v/0.3.0/reference/rendering/#citry-citryrender).

Calling a [Component](/v/0.3.0/reference/component/#citry-component) subclass gives you a [CitryElement](/v/0.3.0/reference/rendering/#citry-citryelement), so you never call `render()` on a component yourself. Real component instances are created only inside the render pipeline.
...
Each strategy suits a different context: a full HTML document, a smaller standalone piece, an HTML fragment for a live web integration, or no handling at all. For the full picture of how JS and CSS are collected and placed, see [JS and CSS dependencies](/v/0.3.0/advanced/js-and-css-dependencies/) and [HTML fragments](/v/0.3.0/advanced/html-fragments/). The [DepsStrategy](/v/0.3.0/reference/rendering/#citry-depsstrategy) and [DepsPosition](/v/0.3.0/reference/rendering/#citry-depsposition) symbols name these choices.
...
- Through provide and inject, for values shared down a whole subtree without threading them through every call. See [Provide and inject](/v/0.3.0/concepts/provide-and-inject/).
- As slot content, markup the parent fills in. Slots have their own page under [Slots](/v/0.3.0/concepts/slots/).

## Building a render by hand

Since a [CitryRender](/v/0.3.0/reference/rendering/#citry-citryrender) is a plain object, you can construct one from ordered parts, where each part is a string or another nested [CitryRender](/v/0.3.0/reference/rendering/#citry-citryrender). Serializing joins them in order and recurses into nested renders:
...
For the full API of every object named here, see the Rendering reference category, including [CitryElement](/v/0.3.0/reference/rendering/#citry-citryelement), [CitryRender](/v/0.3.0/reference/rendering/#citry-citryrender), and [CitryContext](/v/0.3.0/reference/rendering/#citry-citrycontext).
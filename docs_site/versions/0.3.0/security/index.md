---
title: Security
url: https://citry.dev/v/0.3.0/security/
...
Use [`Citry.events.configure`](/v/0.3.0/reference/browser-apis/#citry-events-configure) to tell the browser
...
For the handler and State workflow, see [Server events](/v/0.3.0/events/). The
[direct event routes](/v/0.3.0/events/http/) page covers the HTTP-facing cases.
...
pass the result to the template. See [Expressions](/v/0.3.0/syntax/expressions/) for
the full pattern.
...
[Alpine runtime](/v/0.3.0/advanced/alpine-runtime/#content-security-policy) for the
...
the sandbox off with [Citry](/v/0.3.0/reference/citry/#citry-citry) and `sandbox_expressions=False`. This

...
url: https://citry.dev/v/0.4.4/getting-started/your-first-component/
...
The [`template`](/v/0.4.4/reference/component/#citry-component-template) is ordinary HTML with one Citry tag:
...
[`<c-slot />`](/v/0.4.4/reference/builtins/#c-slot) marks the place where the card's
...
- [`Kwargs`](/v/0.4.4/reference/component/#citry-component-kwargs) lists the `name=value` options you can
  choose when you use the Card.
  This card has one option, `accent`, which chooses the border color.
- [`Slots`](/v/0.4.4/reference/component/#citry-component-slots) lists places where text or HTML can go. This
  card has one place, `default`, for everything written between `<c-Card>` and
  `</c-Card>`.

The [`css_data()`](/v/0.4.4/reference/component/#citry-component-css-data) method sends the chosen accent to
CSS as `--accent`.
...
The [`css`](/v/0.4.4/reference/component/#citry-component-css) block travels with the Card. Citry adds it to
...
!!! note
    ...
    [Inputs and validation](/v/0.4.4/concepts/inputs-and-validation/) to add runtime
    checks.
...
You can [open the Card recipe](/v/0.4.4/examples/card/) to compare the component,

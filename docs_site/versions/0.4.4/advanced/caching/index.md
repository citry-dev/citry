...
url: https://citry.dev/v/0.4.4/advanced/caching/
...
same value wrapped in [`Const`](/v/0.4.4/reference/rendering/#citry-const). `<c-cache>` unwraps `Const` from
...
[`CacheKeyError`](/v/0.4.4/reference/cache-keys/#citry-ext-cache-cachekeyerror). This includes slot defaults,
...
[`Citry.clear`](/v/0.4.4/reference/citry/#citry-citry-clear) advances local invalidation state. It also
...
[Cache backends](/v/0.4.4/advanced/cache-backends/) covers capacity, shared stores,
...
- [Cache backends](/v/0.4.4/advanced/cache-backends/) for in-process and shared
  storage.
- [Performance](/v/0.4.4/advanced/performance/) for reusing stable values and pure
  component bodies inside an ordinary render.
- [Security](/v/0.4.4/security/) for template and Events trust boundaries.
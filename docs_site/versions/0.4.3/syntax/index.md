...
url: https://citry.dev/v/0.4.3/syntax/
...
[Alpine in templates](/v/0.4.3/syntax/alpine/).
...
- `{{ expression }}`: insert a
  [Python value](/v/0.4.3/syntax/expressions/)
- `c-title="heading"`: set a Python value as
  [attribute or component input](/v/0.4.3/syntax/dynamic-attributes/)
- `c-if`, `c-for`: [conditions and loops](/v/0.4.3/syntax/control-flow/)
- `x-*`, `@event`, `:name`: [Alpine behavior](/v/0.4.3/syntax/alpine/)
- `<c-Card>`, `<c-slot>`:
  [components](/v/0.4.3/concepts/components/) and
  [built-in tags](/v/0.4.3/reference/builtins/)
- `{# ... #}`, `<c-raw>`:
  [comments or literal text](/v/0.4.3/syntax/comments/)
- `c-body="<>...</>"`:
  [markup through an attribute](/v/0.4.3/syntax/nested-templates/)

Citry also has attributes for browser and server interaction:

- `$c-props`, `@c-*`, and `:c-*` are covered in
[Client interactivity](/v/0.4.3/concepts/client-interactivity/) and [Events](/v/0.4.3/events/)
- `#c-key` and `#c-ignore` are the template's
[template flags](/v/0.4.3/syntax/dynamic-attributes/#c-template-flags). They guide how
an event response updates existing HTML. See also
[Event actions](/v/0.4.3/events/actions/).
...
Start with [Expressions](/v/0.4.3/syntax/expressions/) if you want to insert or compute
a value. Continue to [Attributes](/v/0.4.3/syntax/dynamic-attributes/) when that value

...
url: https://citry.dev/v/0.4.3/advanced/hooks/
...
[`Extension`](/v/0.4.3/reference/extensions/#citry-extension) instead.
...
- an existing [`CitryRender`](/v/0.4.3/reference/rendering/#citry-citryrender);
- a [`Slot`](/v/0.4.3/reference/slots/#citry-slot), called without data; or
- a [`ComponentLike`](/v/0.4.3/reference/component-libraries/#citry-componentlike).
...
[`template_data()`](/v/0.4.3/reference/component/#citry-component-template-data). The hook already has
access to `self.kwargs`, `self.slots`, `self.parent`, and
[`self.inject()`](/v/0.4.3/reference/component/#citry-component-inject).
...
the condition that produced it. See [Caching](/v/0.4.3/advanced/caching/).
...
- on success, `result` is the live
  [`CitryRender`](/v/0.4.3/reference/rendering/#citry-citryrender) and `error` is `None`;
...
[`on_render()` reference](/v/0.4.3/reference/component/#citry-component-on-render) contains the complete
generator protocol.

For ordinary error recovery, prefer the built-in `<c-error-fallback>` tag.
See [Error boundaries](/v/0.4.3/concepts/error-boundaries/). A custom hook is useful
...
document-level CSP surface. See [Extensions](/v/0.4.3/advanced/extensions/) for the
application-wide hook.

## Next steps

- [Component JavaScript and CSS](/v/0.4.3/advanced/js-and-css-dependencies/) covers
  primary assets and per-render data.
- [Dependency files](/v/0.4.3/advanced/dependency-files/) covers libraries, shared
  files, and custom tags.
- [Place JavaScript and CSS](/v/0.4.3/advanced/asset-placement/) controls where the
  collected tags go.
- [Rendering](/v/0.4.3/concepts/rendering/) explains the larger compose, render, and
  serialize process.
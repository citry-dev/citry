...
url: https://citry.dev/v/0.4.1/advanced/caching/
...
same value wrapped in [`Const`](/v/0.4.1/reference/rendering/#citry-const). `<c-cache>` unwraps `Const` from
...
[`CacheKeyError`](/v/0.4.1/reference/cache-keys/#citry-ext-cache-cachekeyerror). This includes slot defaults,
...
[`Citry.clear`](/v/0.4.1/reference/citry/#citry-citry-clear) advances local invalidation state. It also
...
[Cache backends](/v/0.4.1/advanced/cache-backends/) covers capacity, shared stores,
...
- [Cache backends](/v/0.4.1/advanced/cache-backends/) for in-process and shared
  storage.
- [Const optimization](/v/0.4.1/advanced/const-optimization/) for reusing stable
  pieces inside an ordinary render.
- [Security](/v/0.4.1/security/) for template and Events trust boundaries.
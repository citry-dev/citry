...
url: https://citry.dev/v/0.4.1/advanced/html-fragments/
...
[`DepsStrategy`](/v/0.4.1/reference/rendering/#citry-depsstrategy):
...
1. `Card(...)` creates a [`CitryElement`](/v/0.4.1/reference/rendering/#citry-citryelement).
2. `.render()` creates a [`CitryRender`](/v/0.4.1/reference/rendering/#citry-citryrender).
...
URL. Mount one of Citry's [web framework integrations](/v/0.4.1/web-frameworks/) so
those URLs can be served.

Client-active output includes components that use normal Alpine attributes,
[`$component`](/v/0.4.1/reference/browser-apis/#component), client props, browser or server event handlers, or
...
[`Citry.set_mounted_prefix`](/v/0.4.1/reference/citry/#citry-citry-set-mounted-prefix):
...
The [`deps_position`](/v/0.4.1/reference/rendering/#citry-depsposition) option applies to document and simple
...
[Cache backends](/v/0.4.1/advanced/cache-backends/) for Redis, DiskCache, Django, and
...
[Preserve client-active HTML](/v/0.4.1/advanced/alpine-runtime/#preserve-client-active-html)
for the exact list.

## See also

- [Component JavaScript and CSS](/v/0.4.1/advanced/js-and-css-dependencies/) for a
  component's own browser behavior and styles.
- [Dependency files](/v/0.4.1/advanced/dependency-files/) for URLs and local files.
- [Client interactivity](/v/0.4.1/concepts/client-interactivity/) for browser scope
  and component lifecycles.
- [Event actions](/v/0.4.1/events/actions/) for returning rendered updates from a
  Python handler.
- [Rendering](/v/0.4.1/concepts/rendering/) for render and serialization choices.
---
title: Security
url: https://citry.dev/v/0.5.1/security/
...
[`Script`](/v/0.5.1/reference/dependencies/#citry-ext-dependencies-script). Citry validates and preserves the
...
[`Script`](/v/0.5.1/reference/dependencies/#citry-ext-dependencies-script), including external scripts and inert
JSON manifests, receives it. Every structured
[`Style`](/v/0.5.1/reference/dependencies/#citry-ext-dependencies-style), including external stylesheet links,
...
Use [`Citry.events.configure`](/v/0.5.1/reference/browser-apis/#citry-events-configure) to tell the browser
...
For the handler and State workflow, see [Server events](/v/0.5.1/events/). The
[direct event routes](/v/0.5.1/events/http/) page covers the HTTP-facing cases.
...
- A Python layer runs at evaluation time. It rewrites every variable read,
  ...
  access raises [`SecurityError`](/v/0.5.1/reference/rendering/#citry-securityerror).
...
pass the result to the template. See [Expressions](/v/0.5.1/syntax/expressions/) for
the full pattern.
...
See [Alpine runtime](/v/0.5.1/advanced/alpine-runtime/#use-content-security-policy) for
...
the sandbox off with [Citry](/v/0.5.1/reference/citry/#citry-citry) and `sandbox_expressions=False`. This

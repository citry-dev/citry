...
url: https://citry.dev/v/0.5.1/getting-started/call-python/
...
Start with [Serve the page with FastAPI](/v/0.5.1/getting-started/fastapi/) if the app
...
Citry uses the `@c-*` prefix for events that are meant to be [handled by the server](/v/0.5.1/events/bindings/).
...
!!! warning
    ...
    **DO NOT** blindly trust data in event handlers - anyone can send events. Your application still
    ...
    [Security](/v/0.5.1/security/) for the complete trust boundaries.

## Server event handlers

The handler lives in the component's nested [`Events`](/v/0.5.1/reference/component/#citry-component-events) class:
...
[`actions.Dispatch(...)`](/v/0.5.1/reference/events/#citry-ext-events-actions-dispatch) action:
...
!!! note
    ...
    `@c-click` is a convenience over [`sendEvent()`](/v/0.5.1/reference/browser-apis/#send-event): as such, it starts the handler but does not expose the
    handler's Promise result.

    That is why this example returns [`Dispatch`](/v/0.5.1/reference/events/#citry-ext-events-actions-dispatch) (to trigger a browser event) rather
    than [`actions.Data`](/v/0.5.1/reference/events/#citry-ext-events-actions-data) which would have returned data from a Promise.
...
!!! note
    ...
    For a multi-root component, prefer [`$onEvent`](/v/0.5.1/reference/browser-apis/#on-event) in an
    Alpine expression or the `onEvent` function in [`$component`](/v/0.5.1/reference/browser-apis/#component).
...
The full button also uses the Citry-specific Alpine magic [`$loading('load_choices')`](/v/0.5.1/reference/browser-apis/#loading):
...
Continue with [event bindings](/v/0.5.1/events/bindings/) for loading and errors, or
[event actions](/v/0.5.1/events/actions/) for the other ways a handler can update the
page.

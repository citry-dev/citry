...
url: https://citry.dev/v/0.5.1/getting-started/build-crud-pages/
...
[`State`](/v/0.5.1/reference/component/#citry-component-state), call queue, loading counters, and handler
errors.
...
This lesson keeps the ID in [`State`](/v/0.5.1/reference/component/#citry-component-state):
...
Inside the row, call [`$loading()`](/v/0.5.1/reference/browser-apis/#loading) and [`$error()`](/v/0.5.1/reference/browser-apis/#error) to get loading/error state
scoped to this row:
...
`$error()` and `$loading()` are also accessible inside the component callback [`$component`](/v/0.5.1/reference/browser-apis/#component) as `error()` and `loading()`. Note, these functions return [Alpine reactive objects](https://alpinejs.dev/advanced/reactivity){: target="_blank" rel="noopener"}, and the accessing logic must be wrapped in `effect()`:
...
When you use [`onEvent`](/v/0.5.1/reference/browser-apis/#on-event), Citry smartly passes that Dispatch action to the component instance whose handler
...
The [`Render`](/v/0.5.1/reference/events/#citry-ext-events-actions-render) action replaces the contents of `#task-rows` with a `TaskRows`
component containing only the matching rows. It leaves `TaskList` and both
filter controls in place. The [`Dispatch`](/v/0.5.1/reference/events/#citry-ext-events-actions-dispatch) action updates that existing list
...
That's why the server responds with a [`Dispatch`](/v/0.5.1/reference/events/#citry-ext-events-actions-dispatch) action - this works around the `@c-click`'s limitation:
...
4. In the browser, separate [`onEvent()`](/v/0.5.1/reference/browser-apis/#on-event) callback is registered in `TaskList` to listen for this event.
5. Inside the `onEvent()`, we can access the data sent with this event.

There is a simpler way: When you want to trigger a server event AND get data back from the server, you can instead use [`sendEvent()`](/v/0.5.1/reference/browser-apis/#send-event) together with [`actions.Data`](/v/0.5.1/reference/events/#citry-ext-events-actions-data).

`sendEvent()` returns a Promise that resolves to the data returned by
[`actions.Data`](/v/0.5.1/reference/events/#citry-ext-events-actions-data):
...
- Use [Examples](/v/0.5.1/examples/) when you want working code for a specific task.
- Read [Docs](/v/0.5.1/getting-started/installation/) when you want a concept or guided
  workflow.
- Read [Reference](/v/0.5.1/reference/) when you need the exact API for a class, method, or return action.
- Install [Citry UI](/v/0.5.1/ui-library/), a library of reusable UI components.
- Install [Citry linter](/v/0.5.1/ide/vscode/) for your IDE, to get syntax highlight, diagnostics, and more.
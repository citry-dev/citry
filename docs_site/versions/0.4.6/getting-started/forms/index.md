...
url: https://citry.dev/v/0.4.6/getting-started/forms/
...
The Python type hint `SignupIn` describes the expected input shape. The [Forms guide](/v/0.4.6/events/forms/) covers larger input
shapes and validation patterns.

## Reject input in Python

The handler checks the cleaned address and raises
[`EventError`](/v/0.4.6/reference/events/#citry-ext-events-eventerror) when the domain is wrong:
...
[`$error('submit')?.message`](/v/0.4.6/reference/browser-apis/#error) if you want a message for the whole
form. The
[`fields`](/v/0.4.6/reference/events/#citry-ext-events-eventerror-fields) mapping adds messages for
...
[`$loading('submit')`](/v/0.4.6/reference/browser-apis/#loading):
...
The listener is on the `SignupForm` root on purpose. [`Dispatch`](/v/0.4.6/reference/events/#citry-ext-events-actions-dispatch) fires the
...
[`$onEvent`](/v/0.4.6/reference/browser-apis/#on-event) or the `onEvent` member from
[`$component`](/v/0.4.6/reference/browser-apis/#component) to listen by component instance instead. The
[event actions guide](/v/0.4.6/events/actions/#choose-where-to-listen-for-dispatch)

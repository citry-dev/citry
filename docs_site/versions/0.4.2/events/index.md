---
title: Server events
url: https://citry.dev/v/0.4.2/events/
...
handler, declare the small amount of [`State`](/v/0.4.2/reference/component/#citry-component-state) that may
...
[Web frameworks](/v/0.4.2/web-frameworks/) for the other mounting adapters.
...
Every public method declared on the component's own [`Events`](/v/0.4.2/reference/component/#citry-component-events) class is callable.
An underscore-prefixed method is a private helper or configuration hook.

## Choose your next step

- [Keep State between calls](/v/0.4.2/events/state/) when a later handler needs values
  from the current component.
- [Handle and validate forms](/v/0.4.2/events/forms/) when named controls should become
  typed Python data.
- [Bind events in templates](/v/0.4.2/events/bindings/) for `@c-*`, `:c-*`, polling,
  loading feedback, and errors.
- [Event actions](/v/0.4.2/events/actions/) for renders, browser
  events, history changes, and stable update targets.
- [Use event routes directly](/v/0.4.2/events/http/) for GET handlers, native forms,
...
- [Component.View](/v/0.4.2/guides/migrate-from-component-view/) begins with the
  verb-shaped compatibility route, then moves to named handlers.
- [django-unicorn](/v/0.4.2/guides/migrate-from-django-unicorn/) maps public State,
  model bindings, validation, and browser calls.
- [Tetra](/v/0.4.2/guides/migrate-from-tetra/) maps public methods, promise results,
  Alpine behavior, and the client callback channel.
- [livecomponents](/v/0.4.2/guides/migrate-from-livecomponents/) uses a two-step State
  migration: server-held first, signed per component when appropriate.

Use the [Events migration parity matrix](/v/0.4.2/guides/events-migration-parity/) to

...
url: https://citry.dev/v/0.4.2/events/bindings/
...
controls to [`State`](/v/0.4.2/reference/component/#citry-component-state). Both are compiled with the
...
[Client interactivity](/v/0.4.2/concepts/client-interactivity/#send-events-up-from-a-component-tag)
...
Not every click needs Python. [`$state`](/v/0.4.2/reference/browser-apis/#state) is reactive, so a local button
...
| Magic | Use |
|---|---|
| [`$state`](/v/0.4.2/reference/browser-apis/#state) | Read reactive public State or write a field allowed by `_model`. A write rides the next call from this component. |
| [`$loading()`](/v/0.4.2/reference/browser-apis/#loading) | Test whether any call from this component is queued or running. |
| [`$loading('save')`](/v/0.4.2/reference/browser-apis/#loading) | Test only the named handler. |
| [`$error()`](/v/0.4.2/reference/browser-apis/#error) | Read the newest retained error across this component's handlers, or `null`. |
| [`$error('save')`](/v/0.4.2/reference/browser-apis/#error) | Read only the named handler's retained error. |
| [`$sendEvent(name, args?)`](/v/0.4.2/reference/browser-apis/#send-event) | Send a named event from an Alpine expression. |
| [`$onEvent(name, callback)`](/v/0.4.2/reference/browser-apis/#on-event) | Listen for server-dispatched events and receive an unsubscribe function. |
...
[`actions.Data`](/v/0.4.2/reference/events/#citry-ext-events-actions-data) value. Use `$sendEvent` or
`sendEvent` when browser code needs that one caller's value. Return
[`actions.Dispatch`](/v/0.4.2/reference/events/#citry-ext-events-actions-dispatch) when a declarative

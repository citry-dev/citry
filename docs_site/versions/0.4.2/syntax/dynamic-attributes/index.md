---
title: Attributes
url: https://citry.dev/v/0.4.2/syntax/dynamic-attributes/
...
[expression](/v/0.4.2/syntax/expressions/) and removes the `c-` from the attribute
...
errors. The bare `c-else` and `c-empty` [control-flow markers](/v/0.4.2/syntax/control-flow/) are the two
exceptions.
...
Attribute names and values are HTML-escaped. The value can opt-out of HTML-escaping, see [Bypass HTML escape](/v/0.4.2/syntax/expressions/#bypass-html-escape).

### Components

On a component tag, an attribute is the [component's Python input](/v/0.4.2/concepts/inputs-and-validation/). A static value
...
Read
[Alpine in templates](/v/0.4.2/syntax/alpine/) for browser-side attributes and
[Client interactivity](/v/0.4.2/concepts/client-interactivity/) for values that cross
...
The Alpine expression in `$c-props` must return a JavaScript object. This object must match child's `props` declaration. See [Client interactivity](/v/0.4.2/concepts/client-interactivity#pass-client-props-down).
...
Read more on [Alpine events in Citry](/v/0.4.2/concepts/client-interactivity/#send-events-up-from-a-component-tag).

### `@c-event`

Alpine's event handlers run in the browser. You can instead trigger [server event handlers](/v/0.4.2/events/) by prefixing the event name with `c-`, eg `@c-click`. So:
...
Read more about [Binding events in templates](/v/0.4.2/events/bindings/).
...
The merging rules above only apply to plain HTML elements (including [`<c-element>`](/v/0.4.2/reference/builtins/#c-element)). On a component tag,
...
See
[passing HTML attributes through a component](/v/0.4.2/concepts/client-interactivity/#pass-arbitrary-html-attributes-explicitly).
...
Most structural built-in tags, including [`<c-if>`](/v/0.4.2/reference/builtins/#c-if) and
[`<c-for>`](/v/0.4.2/reference/builtins/#c-for), do not accept an attribute spread. Put `c-bind` on the
...
[Spread slot and fill settings](/v/0.4.2/concepts/slots/#spread-slot-and-fill-settings).
...
In Citry, whether you can pass extra attributes to a component is decided by the [`Kwargs`](/v/0.4.2/reference/component/#citry-component-kwargs) class:
...
For more details see  [Forward HTML attributes](/v/0.4.2/advanced/html-attributes/).

## `:c-*` State bind

A `:c-*` attribute connects an input field on the page (such as `<input>` or `<input type="checkbox">`) to a field of the component's
server-side [`State`](/v/0.4.2/reference/component/#citry-component-state). Citry syncs the two values, so you don't have to.
...
you used `:c-*` on an `<input>` element, and automatically chooses the correct approach. For a list of all supported elements, see [Bind controls to State](/v/0.4.2/events/bindings/#bind-controls-to-state).
...
- `:c-query="refresh"`
    ...
    - calls the `refresh` [event handler](/v/0.4.2/events/) on user input
...
[Bind controls to State](/v/0.4.2/events/bindings/#which-elements-you-can-bind).
...
Read [Bind controls to State](/v/0.4.2/events/bindings/#bind-controls-to-state) for the
full element list and more. See [Keep State between calls](/v/0.4.2/events/state/) for declaring the State fields.
...
There are currently two flags, and both relate to [how the DOM updates](/v/0.4.2/events/actions/#preserve-identity-when-lists-or-parents-re-render) when an event handler re-renders a page:
...
[Preserve identity when lists or parents re-render](/v/0.4.2/events/actions/#preserve-identity-when-lists-or-parents-re-render).
...
[Leave a browser-owned subtree alone](/v/0.4.2/events/actions/#leave-a-browser-owned-subtree-alone)

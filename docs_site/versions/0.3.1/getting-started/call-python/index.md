...
url: https://citry.dev/v/0.3.1/getting-started/call-python/
...
Start with [Serve the page with FastAPI](/v/0.3.1/getting-started/fastapi/) if the app
...
Citry uses the `@c-*` prefix for events that are meant to be [handled by the server](/v/0.3.1/events/bindings/).
...
!!! warning
    ...
    **DO NOT** blindly trust data in event handlers - anyone can send events. Your application still
    ...
    [Security](/v/0.3.1/security/) for the complete trust boundaries.

## Run the matching Python method

The handler lives in the component's nested [`Events`](/v/0.3.1/reference/events/#citry-events) class:
...
[`actions.Dispatch(...)`](/v/0.3.1/reference/events/#citry-ext-events-actions-dispatch) action:
...
The full button also uses the Citry-specific Alpine magic [`$loading('load_choices')`](/v/0.3.1/reference/browser-apis/#loading):
...
Continue with [event bindings](/v/0.3.1/events/bindings/) for loading and errors, or
[event actions](/v/0.3.1/events/actions/) for the other ways a handler can update the
page.

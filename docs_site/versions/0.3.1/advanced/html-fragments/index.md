...
url: https://citry.dev/v/0.3.1/advanced/html-fragments/
...
[`CitryElement`](/v/0.3.1/reference/rendering/#citry-citryelement). `.render()` returns a
[`CitryRender`](/v/0.3.1/reference/rendering/#citry-citryrender). Its `serialize` method returns the final
HTML string. The `deps_strategy` argument is a [`DepsStrategy`](/v/0.3.1/reference/rendering/#citry-depsstrategy)
...
[Web frameworks](/v/0.3.1/web-frameworks/) for the adapters (FastAPI, Flask, Django,
...
- `deps_position` (the [`DepsPosition`](/v/0.3.1/reference/rendering/#citry-depsposition) argument) only
...
- If you run more than one worker, point them at a shared cache backend. The
  ...
  `DjangoCache` for this; see [Caching](/v/0.3.1/advanced/caching/).
- A [`Dependency`](/v/0.3.1/reference/dependencies/#citry-ext-dependencies-dependency) that points at a local file has no URL, so
...
- Preserve Citry's ownership comments and manifest tags when an HTML minifier,
  ...
  [Alpine runtime](/v/0.3.1/advanced/alpine-runtime/#preserve-client-graph-markers).

## See also

- [JS and CSS dependencies](/v/0.3.1/advanced/js-and-css-dependencies/) for how a
  component declares its scripts and styles.
- [Client interactivity](/v/0.3.1/concepts/client-interactivity/) for activation and scope ownership.
- [Event actions](/v/0.3.1/events/actions/) for handlers that return and apply
  rendered updates.
- [Rendering](/v/0.3.1/concepts/rendering/) for the render and serialize model.
- [Web frameworks](/v/0.3.1/web-frameworks/) to mount the routes that serve fragment
  assets.
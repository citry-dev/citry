...
url: https://citry.dev/v/0.3.1/advanced/dynamic-components/
...
like any other [dynamic attribute](/v/0.3.1/syntax/dynamic-attributes/).
...
- A **registered component name** as a string, looked up in this component's
  [`citry`](/v/0.3.1/reference/citry/#citry-citry) instance.
- A **component class** (a [`Component`](/v/0.3.1/reference/component/#citry-component) subclass), used
  directly.
...
rendered [`CitryElement`](/v/0.3.1/reference/rendering/#citry-citryelement) you want to place, print it with
...
unregistered name raises [`NotRegistered`](/v/0.3.1/reference/citry/#citry-notregistered) with a hint to
...
- **No nested-template attribute values on the dynamic path.** An attribute
  ...
  `c-foo="<b>{{ x }}</b>"`) resolves to a [`CitryRender`](/v/0.3.1/reference/rendering/#citry-citryrender)
...
[`c-bind` spread](/v/0.3.1/syntax/dynamic-attributes/)) whenever the target depends on
your data:
...
they render. [Provide and inject](/v/0.3.1/concepts/provide-and-inject/) flow straight
...
[`Citry`](/v/0.3.1/reference/citry/#citry-citry) instance, so you cannot register your own component
under those names.

For choosing markup based on data without swapping the whole tag, see
[Control flow](/v/0.3.1/syntax/control-flow/); for the attribute syntax `c-is` builds
on, see [Dynamic attributes](/v/0.3.1/syntax/dynamic-attributes/).
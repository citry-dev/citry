...
Walk a griffe ``Expr`` tree (the parsed form of a type hint like ``Component |
None`` or ``dict[str, Slot]``) and emit HTML where each referenced type becomes
a link when a resolver can place it. Dotted attribute paths collapse to their
leaf name, so a fully-qualified ``citry.slot.Slot`` renders as a linked
``Slot``.

The resolver is injected so this module stays independent of where URLs come
...
def _render_attribute(node: object, resolve: Callable[[str, str], str | None]) -> str:
    # Dotted path (a.b.C): show and link only the leaf, keyed on its full path.
...
# Dispatch on the griffe node's class name so we do not import griffe's Expr
# types here (they are injected via the annotation object at call time).

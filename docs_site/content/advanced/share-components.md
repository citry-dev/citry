...
package's explicit manifest into each [`Citry`][citry.Citry] instance that
should use it.

The public pieces are:

- [`LibraryComponent`][citry.LibraryComponent], the engine-neutral definition;
- [`ComponentLibrary`][citry.ComponentLibrary], the ordered package manifest;
- [`Citry.register_library()`][citry.Citry.register_library], the application
  startup operation; and
- [`LibraryInstallation`][citry.LibraryInstallation], the committed
  installation handle.
...
subclass the runtime [`SlotData`][citry.SlotData] record.
...
Registration creates a distinct concrete [`Component`][citry.Component] class
...
tags. Name collisions raise [`AlreadyRegistered`][citry.AlreadyRegistered]
...
See [Slots](/concepts/slots/#scoped-slots-passing-data-to-the-fill) for the
...
[`LibraryComponentInvocation`][citry.LibraryComponentInvocation], which follows
Citry's structural [`ComponentLike`][citry.ComponentLike] protocol. At render
...
[`LibraryComponentContextError`][citry.LibraryComponentContextError] when no
...
implementing [`ComponentLike`][citry.ComponentLike]:
...
After [`Citry.clear()`][citry.Citry.clear], the handle is retired and component
lookup raises [`LibraryInstallationStale`][citry.LibraryInstallationStale].

...
Inside the JS, [`$component(...)`][$component] registers a callback that citry
...
[`Style`][citry.ext.dependencies.Style] when an asset needs inline content or
...
`serialize` method takes a `deps_strategy` ([`DepsStrategy`][citry.DepsStrategy])
...
- `"fragment"`: emits a URL manifest for insertion into an already-loaded page.
  See [HTML fragments](/advanced/html-fragments/).
- `"ignore"`: inserts no dependency tags at all.

A `deps_position` ([`DepsPosition`][citry.DepsPosition]) of `"smart"` (the
...
citry gives you the final say over them. Each asset is a [`Script`][citry.ext.dependencies.Script]
or [`Style`][citry.ext.dependencies.Style] object holding either inline `content` or a `url`, plus
...
`on_dependencies` method on an [Extension][citry.Extension]. It receives an
...
See [Extensions](/advanced/extensions/) for how extensions are written and
installed.
...
When you build a [`Script`][citry.ext.dependencies.Script] yourself, as a
...
- [HTML fragments](/advanced/html-fragments/) covers the `"fragment"` strategy
  for HTMX-style page updates.
- [Add browser behavior](/getting-started/browser-interactivity/) introduces
  component JavaScript through one small interaction.
- [Web frameworks](/web-frameworks/) shows how to mount Citry so local assets

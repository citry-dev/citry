...
Both use the cache backend configured on your [Citry][citry.Citry] instance.
...
content, caching raises [`CacheKeyError`][citry.ext.cache.CacheKeyError] unless
...
Pass `cache=` when you construct [Citry][citry.Citry]. It accepts a backend
...
`app.cache`. A backend implements the [CitryCache][citry.CitryCache] protocol:
...
[InMemoryCache][citry.InMemoryCache] is thread-safe but process-local. Use a
...
- [Performance](/advanced/performance/) for `Const`, which reuses stable pieces
  inside ordinary renders rather than caching a whole subtree.
- [Extensions](/advanced/extensions/) for extension lifecycle and metadata.

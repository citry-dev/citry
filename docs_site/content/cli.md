...
[Your first component](/getting-started/your-first-component/)), with a
[citry.Component][citry.Component] subclass, a `Kwargs` typed input class, a
...
([citry.citry][citry.citry]). To run against an engine you constructed
...
- `--app` is recognized only as the very first argument (`--app VALUE` or
  ...
  and errors out.
- The target must be a [citry.Citry][citry.Citry] *instance*, not the class.
...
[citry.ComponentCatalog][citry.ComponentCatalog] as compact JSON:
...
A command is a [citry.ExtensionCommand][citry.ExtensionCommand] subclass listed
in your extension's `commands`. Declare its arguments as
[citry.CommandArg][citry.CommandArg] values and define `handle`, which receives

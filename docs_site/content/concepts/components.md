...
subclass [`Component`][citry.Component], give it a template, and describe how the
...
[Expressions](/syntax/expressions/) for what you can put inside them.
...
[Typing and validation](/concepts/typing-and-validation/) for what the `Kwargs`
class buys you.
...
**not** return a `Component` instance. It returns a [`CitryElement`][citry.CitryElement],
...
`render()` returns a [`CitryRender`][citry.CitryRender] (an object, not a
...
live. For the full picture see [Rendering](/concepts/rendering/).
...
named `slots`. See [Slots](/concepts/slots/).
...
[Build a page from components](/getting-started/build-page/).

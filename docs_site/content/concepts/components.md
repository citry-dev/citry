...
[`Component`][citry.Component]. Most components bring together:
...
Set [`template`][citry.Component.template] to the markup you want the
...
[`Kwargs`][citry.Component.Kwargs]. By default, Citry makes those values
...
default [`template_data()`][citry.Component.template_data] returns the
...
[Inputs and validation](/concepts/inputs-and-validation/) for required
...
[`TemplateData`][citry.Component.TemplateData] when you also want Citry to
...
Python code. Set [`template_file`][citry.Component.template_file] when you
...
[`on_render()`][citry.Component.on_render] hook supplies its complete output.

## Compose first, render when you need HTML

Calling a component class returns a
[`CitryElement`][citry.CitryElement], not a live `Component` instance and not
...
[Slots](/concepts/slots/) for template and Python examples.
...
fresh rendered occurrence. [Rendering](/concepts/rendering/) explains when to

...
to whatever fills that slot. See [Slots](/concepts/slots/) for how slot content
is rendered.
...
Alpine expressions use the corresponding [`$provide`][$provide],
[`$inject`][$inject], and [`$unprovide`][$unprovide] magics. Establish values
...
as shown above. See [Slots](/concepts/slots/) for that difference.
...
[Dynamic attributes](/syntax/dynamic-attributes/) for that syntax, and `c-bind`
...
The [Component][citry.Component] `provide`, `inject`, and `unprovide` methods,
...
fit in a render, see [Rendering](/concepts/rendering/).

...
[Expressions](/syntax/expressions/)) and strips exactly one leading `c-` from
the rendered name.
...
shorthands `c-else` and `c-empty` (see [Control flow](/syntax/control-flow/)).
...
[normalize_style][citry.normalize_style],
[parse_string_style][citry.parse_string_style],
[merge_attrs][citry.merge_attrs], and [format_attrs][citry.format_attrs].
...
See [HTML attributes](/concepts/html-attributes/) for how these fit into

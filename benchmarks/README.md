...
The README and docs-site chart use `publication.py`, which measures the current
runtime and the public `Component.simple = True` API in one controlled run.
...
Reproduce the report and both PNG copies:
...
[Raw observations](results/publication-20260910-release-0.5.0.json) include wall/CPU samples,
GC counters, source hashes and output digests;

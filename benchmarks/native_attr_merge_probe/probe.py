"""Measure the native attribute merge loop against the ordinary Python loop."""
...
from benchmarks.native_attr_merge_probe.adapter import ARTIFACT, NATIVE, ORIGINAL, install, merge  # noqa: E402
...
def capture_counts(module: Any, data: Any, *, changed: bool) -> dict[str, Any]:
    ...
    expected = merge if changed else ORIGINAL
    if nodes._merge_resolved_attrs is not expected or attrs._merge_resolved_attrs is not expected:
        raise RuntimeError("The intended attribute merger is not active")
    counts = {"native": 0, "fallback": 0}
    original_native = NATIVE.merge
    ...
    NATIVE.merge = counted
    try:
        module.render(data)
    finally:
        NATIVE.merge = original_native
    if changed and not counts["native"]:
        raise RuntimeError("The candidate did not execute the native merger")
    return {"snapshot_table_types": tables, "merge_calls": counts}
...
def equivalence(size: str = "lg") -> dict[str, Any]:
    ...
    try:
        ...
        if not traces[0] or traces[0] != traces[1] or outputs[0] != outputs[1]:
            raise RuntimeError("Native attribute merging changed fixture HTML or ownership snapshots")
    finally:
        OwnershipGraph.snapshot = original_snapshot
...
def main() -> None:
    """Retain balanced process comparisons for native attribute merging."""
    ...
    report = {
        ...
        "size": args.size,
        "experiment": "merge attribute contributions in one native call",

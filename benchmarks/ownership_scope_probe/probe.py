"""Measure smaller ownership scope objects against generator context managers."""
...
from benchmarks.ownership_scope_probe import adapter  # noqa: E402
...
def capture_counts(module: Any, data: Any, *, changed: bool) -> dict[str, Any]:
    ...
    for name, factory in expected.items():
        if getattr(OwnershipGraph, name) is not factory:
            raise RuntimeError("The intended ownership scope factory is not active")
    counts = dict.fromkeys(adapter.NAMES, 0)

    def counted(name: str, factory: Any) -> Any:
    ...
    try:
        for name, factory in expected.items():
            setattr(OwnershipGraph, name, counted(name, factory))
        module.render(data)
    finally:
        for name, factory in expected.items():
            setattr(OwnershipGraph, name, factory)
    return {"snapshot_table_types": tables, "scope_counts": counts}
...
def equivalence(size: str = "lg") -> dict[str, Any]:
    ...
    try:
        ...
        if not traces[0] or traces[0] != traces[1] or outputs[0] != outputs[1]:
            raise RuntimeError("Ownership scope objects changed fixture HTML or ownership snapshots")
    finally:
        OwnershipGraph.snapshot = original_snapshot
...
def main() -> None:
    """Retain balanced process comparisons for ownership scope objects."""
    ...
    report = {
        ...
        "size": args.size,
        "experiment": "use ownership scope objects instead of generator context managers",
        ...
        "hashes": {
            ...
            for path in (
                ...
                Path(__file__).with_name("scopes.py"),
                Path(attrs.__file__),
                Path(nodes.__file__),

"""Measure lazy fallback-slot initialization without changing production."""
...
from citry.ownership import LogicalFillKind, OwnershipGraph  # noqa: E402
from citry.slots import Slot  # noqa: E402

ORIGINAL = nodes._make_body_slot
COUNTS = None


class LazyFallbackSlot(Slot):
    """Initialize a fallback on access, then use the ordinary Slot class."""

    __slots__ = ()

    def __getattribute__(self, name: str) -> Any:
        # Use the existing layout so materialization needs no second Slot object.
        body, context, component_name, slot_name, data_binding, fallback_var, position = object.__getattribute__(
            self, "contents"
        )
        object.__setattr__(self, "__class__", Slot)
        content = nodes._TemplateSlotContent(body, context, component_name, slot_name, data_binding, fallback_var)
        Slot.__init__(
            self,
            body,
...
def candidate(
    ...
    *,
    ...
    """Keep capture eager while delaying the fallback's Python callable."""
    if kind is not LogicalFillKind.FALLBACK:
        return ORIGINAL(
            body,
            context,
            component_name,
    ...
    slot = object.__new__(LazyFallbackSlot)
    object.__setattr__(
        slot, "contents", (body, context, component_name, slot_name, data_binding, fallback_var, position)
    )
    if COUNTS is not None:
        COUNTS["created"] += 1
...
def install(changed: bool) -> None:
    """Switch the sole template-body Slot factory."""
    nodes._make_body_slot = candidate if changed else ORIGINAL


def capture_counts(module: Any, data: Any) -> dict[str, Any]:
    """Count creations and initialization in one separate untimed render."""
    global COUNTS  # noqa: PLW0603 - only enabled outside worker timing
    COUNTS = Counter()
    try:
        module.render(data)
        return dict(COUNTS)
...
def equivalence() -> dict[str, Any]:
    """Compare fixture snapshots while leaving wider API qualification explicit."""
    module = scenario()
    ...
    try:
        ...
        if not traces[0] or traces[0] != traces[1] or outputs[0] != outputs[1]:
            raise RuntimeError("Lazy fallback initialization changed fixture HTML or ownership snapshots")
    finally:
        OwnershipGraph.snapshot = original_snapshot
    ...
    try:
        ...
        install(changed=False)
    if counts.get("created") != 274 or counts.get("initialized", 0) >= counts["created"]:
        raise RuntimeError("The fixture did not leave fallback initialization unused")
    return {"snapshots_compared": len(traces[0]), "snapshots_equal": True, "html_equal": True, "activation": counts}
...
def worker(changed: bool, samples: int, id_base: int) -> dict[str, Any]:
    ...
    if changed and (activation.get("created") != 274 or activation.get("initialized", 0) >= 274):
        raise RuntimeError("Lazy fallback initialization was not active")
    return {
        "candidate": changed,
        "python": sys.version,
...
def main() -> None:
    """Retain balanced process comparisons for lazy fallback initialization."""

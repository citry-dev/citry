"""Measure bounded reuse of immutable source sites across root renders."""
...
ORIGINAL = OwnershipGraph.record_source_location
SITE_TYPE = ownership._SourceSite
SITE_NEW = SITE_TYPE.__new__
SITE_INIT = SITE_TYPE.__init__


def build_site(source_text: str, position: tuple[int, int], origin: str | None) -> Any:
    """Run the same validation and position calculation as the original method."""
...
cached_site = lru_cache(maxsize=256)(build_site)


def shared_site(source_text: str, position: tuple[int, int], origin: str | None) -> Any:
    """Bound shared retention and leave custom values and constructors uncached."""
    if (
        ownership._SourceSite is SITE_TYPE
        and SITE_TYPE.__new__ is SITE_NEW
        and SITE_TYPE.__init__ is SITE_INIT
        and type(source_text) is str
        and len(source_text) <= 8192
        and (origin is None or (type(origin) is str and len(origin) <= 1024))
        and type(position) is tuple
        and len(position) == 2
        and type(position[0]) is int
        and type(position[1]) is int
        and position[0].bit_length() <= 32
        and position[1].bit_length() <= 32
    ):
        return cached_site(source_text, position, origin)
    return build_site(source_text, position, origin)


def candidate() -> Any:
    """Replace only the calculation performed on a graph-local cache miss."""
    source = inspect.getsource(ORIGINAL)
    start = source.index("            source_bytes = source_text.encode()")
    end = source.index("            self._source_site_cache[site_key] = site", start)
    source = source[:start] + "            site = _probe_shared_site(source_text, position, origin)\n" + source[end:]
    ownership.__dict__["_probe_shared_site"] = shared_site
    scope = {}
    # Extracted trusted runtime code keeps the existing occurrence and owner handling.
    exec("from __future__ import annotations\n" + dedent(source), ownership.__dict__, scope)  # noqa: S102
    return scope["record_source_location"]
...
def install(changed: bool) -> None:
    """Switch the source-location recording method without replacing graph caches."""
    OwnershipGraph.record_source_location = CANDIDATE if changed else ORIGINAL


def capture_counts(module: Any, data: Any) -> dict[str, Any]:
    """Measure shared-cache activation in one separate untimed render."""
    before = cached_site.cache_info()
    module.render(data)
    after = cached_site.cache_info()
    if after.hits <= before.hits or after.currsize > 256:
        raise RuntimeError("The shared source-site cache was inactive or exceeded its entry bound")
...
def equivalence() -> dict[str, Any]:
    ...
    try:
        ...
        if not traces[0] or traces[0] != traces[1] or outputs[0] != outputs[1]:
            raise RuntimeError("Shared source sites changed fixture HTML or ownership snapshots")
    finally:
        OwnershipGraph.snapshot = original_snapshot
...
def main() -> None:
    """Retain balanced process comparisons for shared immutable source sites."""

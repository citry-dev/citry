"""Compare complete renders with direct template fills or generic slot calls."""
...
def worker(changed: bool, samples: int, id_base: int) -> dict[str, Any]:
    ...
    try:
        ...
        adapter.COUNTS = None
    if len(snapshots) != 4 or (changed and not counts["direct_calls"]):
        raise RuntimeError("Missing ownership captures or inactive direct template fills")
    return {
        "candidate": changed,
        "python": sys.version,

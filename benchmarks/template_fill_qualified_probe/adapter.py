...
GRAPH_METHODS = {
    ...
    for name in (
        ...
        "supplied_fill_id",
        "capture_slot_call",
        "_capture_python_slot_region",
    )
}
# Each expression names a dependency that the direct path omits or assumes ordinary.
HELPER_PATHS = (
    "nodes._make_body_slot",
    "nodes._bind_fill_data",
    "nodes.add_slot_to_error_message",
    "nodes.Slot",
    "slots.Slot",
    "nodes._TemplateSlotContent",
    "slots._normalize_slot_data",
    "slots.SlotData",
    "slots.SlotData.__init__",
    "slots.SlotData.__new__",
    "slots.SlotContext",
    "slots.SlotContext.__init__",
    "slots.SlotContext.__new__",
    "slots.SlotContext.__getattribute__",
    "slots.SlotContext.data",
    "slots.SlotContext.fallback",
    "slots.SlotContext.provides",
    "ownership._SelectedSupply",
    "ownership._SelectedSupply.__init__",
    "ownership._SelectedSupply.__new__",
    "ownership.capture_current_slot_call",
    "ownership.current_ownership_graph",
    "nodes.current_ownership_graph",
    "ownership.SourceLocationRecord",
    "ownership.SourceLocationRecord.__new__",
    "ownership._SourceSite",
    "ownership._SourceSite.__new__",
    "ownership.LogicalFillRecord",
    "ownership.LogicalFillRecord.__new__",
    "slots.Slot.__init__",
    "slots.Slot.__new__",
    "slots.Slot.__call__",
    "slots.Slot.__getattribute__",
    "slots.Slot.__setattr__",
    "slots.Slot.content_func",
    "nodes._TemplateSlotContent.__init__",
    "nodes._TemplateSlotContent.__new__",
    "nodes._TemplateSlotContent.__call__",
    "nodes._TemplateSlotContent.__getattribute__",
    "nodes._TemplateSlotContent.__setattr__",
    "nodes._TemplateSlotContent._render",
    "nodes._TemplateSlotContent.bind_slot",
    "nodes._TemplateSlotContent._body",
    "nodes._TemplateSlotContent._context",
    "nodes._TemplateSlotContent._data_binding",
    "nodes._TemplateSlotContent._fallback_var",
    "nodes._TemplateSlotContent._slot_ref",
)


def build_helper_check() -> Any:
    """Read current helper identities without allocating tuples on each slot call."""
    namespace = dict(globals())
    conditions = []
    for index, expression in enumerate(HELPER_PATHS):
        name = f"_expected_{index}"
        namespace[name] = eval(expression, namespace)  # noqa: S307 - fixed internal expressions
        conditions.append(f"{expression} is {name}")
    source = "def helpers_unchanged():\n    return (" + " and ".join(conditions) + ")\n"
    exec(compile(source, "<template-fill-helper-check>", "exec"), namespace)  # noqa: S102
    return namespace["helpers_unchanged"]


helpers_unchanged = build_helper_check()
...
def eligible(node: Any, context: Any, fill: Any, name: Any) -> bool:
    ...
    return not (
        ...
        or type(node.position) is not tuple
        or len(node.position) != 2
        or any(type(value) is not int for value in node.position)
        or type(content._context) is not nodes.CitryContext
        or type(context.provides) is not dict
        or type(content._context.provides) is not dict
        or not helpers_unchanged()
        or ownership.current_ownership_graph() is not graph
...
def invoke(fill: Any, data: Any, fallback: Any, provides: Any) -> Any:
    """Use the ordinary call when preparation retained the fallback Slot."""

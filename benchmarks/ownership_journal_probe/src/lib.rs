//! Experimental combined invocation/queue storage. Not a shipped Citry API.
...
impl Journal {
    ...
    fn ancestors(
        &self,
        py: Python<'_>,
        instances: PyRef<'_, storage::RecordTable>,
        edges: PyRef<'_, storage::RecordTable>,
        regions: PyRef<'_, storage::RecordTable>,
        seeds: &Bound<'_, PyAny>,
    ) -> PyResult<Py<PyAny>> {
        if instances.width != 8 || edges.width != 5 || regions.width != 11 {
            return Err(PyValueError::new_err(
                "ancestry table widths must be eight, five and eleven",
            ));
        }
        ancestry::ancestors(self, py, &instances, &edges, &regions, seeds)
    }
...
#[pymodule]
fn citry_ownership_journal_probe(module: &Bound<'_, PyModule>) -> PyResult<()> {

"""Measure direct ownership positions on the ordinary append-only path."""

from __future__ import annotations

import argparse
import ast
import gc
import hashlib
import inspect
import itertools
import json
import os
import random
import statistics
import subprocess
import sys
import time
from collections import Counter
from pathlib import Path
from textwrap import dedent
from typing import Any

ROOT = Path(__file__).resolve().parents[2]
sys.path[:0] = [str(ROOT), str(ROOT / "packages/py/citry"), str(ROOT / "packages/py/citry_core")]

from benchmarks.ownership_journal_probe.probe import scenario  # noqa: E402

import citry.citry_render as renders  # noqa: E402
import citry.component_render as components  # noqa: E402
import citry.util.id as ids  # noqa: E402
import citry_core._rust as native  # noqa: E402
from citry import ownership  # noqa: E402
from citry.ownership import OwnershipGraph  # noqa: E402

INDEX_TABLES = {
    "_invocation_index": ("_component_invocations", "id"),
    "_fill_index": ("_logical_fills", "id"),
    "_region_index": ("_physical_regions", "id"),
    "_queue_index": ("_render_queue", "invocation_id"),
}
ORIGINALS = {}
CANDIDATES = {}
TRANSFORMS = Counter()


def index_name(node: ast.AST) -> str | None:
    """Recognize only the four index attributes on the current graph."""
    if isinstance(node, ast.Attribute) and isinstance(node.value, ast.Name) and node.value.id == "self":
        return node.attr if node.attr in INDEX_TABLES else None
    return None


class DirectPositions(ast.NodeTransformer):
    """Replace only indexed reads, writes and membership in the selected maps."""

    def visit_Assign(self, node: ast.Assign) -> Any:
        if (
            len(node.targets) == 1
            and isinstance(node.targets[0], ast.Subscript)
            and index_name(node.targets[0].value) is not None
        ):
            TRANSFORMS["writes_removed"] += 1
            return None
        return self.generic_visit(node)

    def visit_Subscript(self, node: ast.Subscript) -> Any:
        if isinstance(node.ctx, ast.Load) and index_name(node.value) is not None:
            TRANSFORMS["reads_replaced"] += 1
            return ast.copy_location(ast.BinOp(left=node.slice, op=ast.Sub(), right=ast.Constant(1)), node)
        return self.generic_visit(node)

    def visit_Compare(self, node: ast.Compare) -> Any:
        if len(node.ops) == 1 and isinstance(node.ops[0], (ast.In, ast.NotIn)):
            name = index_name(node.comparators[0])
            if name is not None:
                TRANSFORMS["membership_replaced"] += 1
                table, _ = INDEX_TABLES[name]
                limit = ast.Call(
                    func=ast.Name(id="len", ctx=ast.Load()),
                    args=[ast.Attribute(value=ast.Name(id="self", ctx=ast.Load()), attr=table, ctx=ast.Load())],
                    keywords=[],
                )
                result = ast.Compare(left=ast.Constant(0), ops=[ast.Lt(), ast.LtE()], comparators=[node.left, limit])
                if isinstance(node.ops[0], ast.NotIn):
                    result = ast.UnaryOp(op=ast.Not(), operand=result)
                return ast.copy_location(result, node)
        return self.generic_visit(node)


def reject_replay(*_args: Any, **_kwargs: Any) -> Any:
    """Refuse replay rather than silently apply an unqualified position invariant."""
    raise RuntimeError("The direct-position experiment does not support ownership replay")


for method_name, method in list(OwnershipGraph.__dict__.items()):
    if not inspect.isfunction(method):
        continue
    if method_name in {"import_replayed_snapshot", "_capture_replay_mutation", "_restore_replay_mutation"}:
        ORIGINALS[method_name] = method
        CANDIDATES[method_name] = reject_replay
        continue
    source = inspect.getsource(method)
    if not any(name in source for name in INDEX_TABLES):
        continue
    tree = ast.fix_missing_locations(DirectPositions().visit(ast.parse(dedent(source))))
    scope = {}
    exec(  # noqa: S102 - transform trusted local runtime source
        compile("from __future__ import annotations\n" + ast.unparse(tree), "ownership-direct-position", "exec"),
        ownership.__dict__,
        scope,
    )
    ORIGINALS[method_name] = method
    CANDIDATES[method_name] = scope[method_name]


def install(changed: bool) -> None:
    """Switch the transformed ownership methods as one experimental area."""
    for name, method in (CANDIDATES if changed else ORIGINALS).items():
        setattr(OwnershipGraph, name, method)


def assert_dense(graph: Any) -> dict[str, Any]:
    """Check every retained row's position and the four index populations."""
    counts = {}
    for index, (table, field) in INDEX_TABLES.items():
        records = getattr(graph, table)
        if any(getattr(record, field) != position for position, record in enumerate(records, 1)):
            raise RuntimeError(f"The ordinary position invariant failed for {table}")
        counts[index] = {"records": len(records), "index_entries": len(getattr(graph, index))}
    return counts


def capture_counts(module: Any, data: Any) -> dict[str, Any]:
    """Inspect a completed tree outside timing while its graph remains reachable."""
    rendered = module.ProjectPage(**data).render()
    counts = assert_dense(rendered.context.ownership)
    if not all(row["records"] > 0 and row["index_entries"] == 0 for row in counts.values()):
        raise RuntimeError("The direct-position candidate was not active")
    return counts


def equivalence() -> dict[str, Any]:
    """Compare every reached ownership snapshot and the fixture HTML."""
    module = scenario()
    data = module.gen_render_data()
    original_snapshot = OwnershipGraph.snapshot
    traces, outputs = [], []
    try:
        for changed in (False, True):
            install(changed)
            for _ in range(6):
                module.render(data)
            trace = []

            def snapshot(graph: Any, _trace: list[Any] = trace) -> Any:
                assert_dense(graph)
                result = original_snapshot(graph)
                _trace.append(result)
                return result

            OwnershipGraph.snapshot = snapshot
            ids._id_counter = itertools.count()
            outputs.append(module.render(data))
            OwnershipGraph.snapshot = original_snapshot
            traces.append(trace)
        if not traces[0] or traces[0] != traces[1] or outputs[0] != outputs[1]:
            raise RuntimeError("Direct positions changed fixture HTML or ownership snapshots")
    finally:
        OwnershipGraph.snapshot = original_snapshot
        install(changed=False)
    install(changed=True)
    try:
        activation = capture_counts(module, data)
    finally:
        install(changed=False)
    return {
        "snapshots_compared": len(traces[0]),
        "snapshots_equal": True,
        "html_equal": True,
        "activation": activation,
    }


def worker(changed: bool, samples: int, id_base: int) -> dict[str, Any]:
    """Include all complete renders and their GC costs on one private heap."""
    ids._id_base = id_base
    ids._id_counter = itertools.count()
    install(changed)
    module = scenario()
    data = module.gen_render_data()
    for _ in range(6):
        module.render(data)
    before = gc.get_stats()
    observations, digests = [], []
    for index in range(samples):
        ids._id_counter = itertools.count((index + 1) * 1_000_000)
        cpu_start = time.process_time_ns()
        start = time.perf_counter_ns()
        output = module.render(data)
        end = time.perf_counter_ns()
        cpu_end = time.process_time_ns()
        observations.append({"ms": (end - start) / 1_000_000, "cpu_ms": (cpu_end - cpu_start) / 1_000_000})
        digests.append(hashlib.sha256(output.encode()).hexdigest())
        del output
    after = gc.get_stats()
    activation = capture_counts(module, data) if changed else None
    return {
        "candidate": changed,
        "python": sys.version,
        "hash_seed": os.environ.get("PYTHONHASHSEED"),
        "id_base": id_base,
        "gc_enabled": gc.isenabled(),
        "gc_thresholds": gc.get_threshold(),
        "gc_before": before,
        "gc_after": after,
        "native_sha256": hashlib.sha256(Path(native.__file__).read_bytes()).hexdigest(),
        "activation": activation,
        "means_ms": {key: statistics.mean(row[key] for row in observations) for key in ("ms", "cpu_ms")},
        "observations": observations,
        "html_digests": digests,
    }


def main() -> None:
    """Retain balanced process comparisons for direct ownership positions."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--worker", choices=("reference", "candidate"))
    parser.add_argument("--pairs", type=int, default=8)
    parser.add_argument("--samples", type=int, default=80)
    parser.add_argument("--seed", type=int, default=20260913)
    parser.add_argument("--id-base", type=int, default=123456)
    parser.add_argument("--output", type=Path)
    args = parser.parse_args()
    if min(args.pairs, args.samples) < 1:
        parser.error("Pair and sample counts must be positive")
    if args.worker:
        print(json.dumps(worker(args.worker == "candidate", args.samples, args.id_base)))
        return
    if args.output is None:
        parser.error("The parent requires --output")
    checks = equivalence()
    orders = [bool(index % 2) for index in range(args.pairs)]
    random.Random(args.seed).shuffle(orders)  # noqa: S311 - repeatable process order
    pairs = []
    for pair, candidate_first in enumerate(orders):
        variants = {}
        for name in ("candidate", "reference") if candidate_first else ("reference", "candidate"):
            result = subprocess.run(
                [
                    sys.executable,
                    __file__,
                    "--worker",
                    name,
                    "--samples",
                    str(args.samples),
                    "--id-base",
                    str(args.id_base + pair * 10_000_000),
                ],
                cwd=ROOT,
                env={**os.environ, "PYTHONHASHSEED": str(args.seed + pair)},
                capture_output=True,
                text=True,
                check=True,
            )
            variants[name] = json.loads(result.stdout)
        if variants["reference"]["html_digests"] != variants["candidate"]["html_digests"]:
            raise RuntimeError(f"HTML differs in pair {pair}")
        if variants["reference"]["native_sha256"] != variants["candidate"]["native_sha256"]:
            raise RuntimeError("Paired workers loaded different native artifacts")
        savings = {
            key: variants["reference"]["means_ms"][key] - variants["candidate"]["means_ms"][key]
            for key in ("ms", "cpu_ms")
        }
        pairs.append({"pair": pair, "candidate_first": candidate_first, "mean_savings_ms": savings, **variants})
        print(json.dumps({"pair": pair, "mean_savings_ms": savings}), flush=True)
    report = {
        "revision": subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True).strip(),  # noqa: S607
        "experiment_only": True,
        "transforms": dict(TRANSFORMS),
        "methods_replaced": sorted(CANDIDATES),
        "order_seed": args.seed,
        "order": "balanced-random",
        "samples_per_process": args.samples,
        "warmups_per_process": 6,
        "equivalence": checks,
        "hashes": {
            str(path.relative_to(ROOT)): hashlib.sha256(path.read_bytes()).hexdigest()
            for path in (
                Path(__file__).resolve(),
                Path(renders.__file__),
                Path(components.__file__),
                Path(ownership.__file__),
                ROOT / "packages/py/citry/tests/test_benchmark_citry.py",
            )
        },
        "median_process_pair_mean_savings_ms": {
            key: statistics.median(pair["mean_savings_ms"][key] for pair in pairs) for key in ("ms", "cpu_ms")
        },
        "joint_favorable_process_pairs": sum(
            all(value > 0 for value in pair["mean_savings_ms"].values()) for pair in pairs
        ),
        "pairs": pairs,
    }
    args.output.write_text(json.dumps(report, indent=2) + "\n")
    print(json.dumps({key: value for key, value in report.items() if key != "pairs"}, indent=2))


if __name__ == "__main__":
    main()

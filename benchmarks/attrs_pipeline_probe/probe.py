"""Measure final-string reuse from live primitive attribute contributions."""
...
from benchmarks.attrs_pipeline_probe.adapter import ORIGINAL, install, render  # noqa: E402
...
PROBE_ARTIFACT = Path(native.__file__)
NATIVE = _ownership
...
def capture_counts(module: Any, data: Any, *, changed: bool) -> dict[str, Any]:
    ...
    expected = render if changed else ORIGINAL
    if nodes.ElementAttrsNode.render is not expected:
        raise RuntimeError("The intended attribute renderer is not active")
    counts = {"collections": 0, "finishes": 0, "cache_hits": 0, "cache_misses": 0}
    originals = adapter.COLLECT_PLAIN, adapter.COLLECT_SPREAD, adapter.FINISH, adapter.CACHE

    def plain(*args: Any) -> Any:
        counts["collections"] += 1
        return originals[0](*args)

    def spread(*args: Any) -> Any:
        counts["collections"] += 1
    ...
    class CountedCache(dict):
        def get(self, key: Any, default: Any = None) -> Any:
            ...
            counts["cache_misses" if result is None else "cache_hits"] += 1
            return result

    adapter.COLLECT_PLAIN, adapter.COLLECT_SPREAD, adapter.FINISH = plain, spread, finish
    adapter.CACHE = CountedCache(originals[3])
    try:
        module.render(data)
        counts["cache_entries"] = len(adapter.CACHE)
    finally:
        adapter.COLLECT_PLAIN, adapter.COLLECT_SPREAD, adapter.FINISH, adapter.CACHE = originals
    if changed and not counts["cache_hits"]:
        raise RuntimeError("The primitive contribution cache did not hit")
    return {"snapshot_table_types": tables, "pipeline_counts": counts}
...
def equivalence(size: str = "lg") -> dict[str, Any]:
    ...
    try:
        ...
        if not traces[0] or traces[0] != traces[1] or outputs[0] != outputs[1]:
            raise RuntimeError("Contribution output reuse changed fixture HTML or ownership snapshots")
    finally:
        OwnershipGraph.snapshot = original_snapshot
...
def main() -> None:
    """Retain balanced process comparisons for contribution output reuse."""
    ...
    report = {
        ...
        "size": args.size,
        "experiment": "reuse final attribute strings from current primitive contributions",

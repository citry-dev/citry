"""Check caller-owned button attributes and content across a real slot boundary."""
...
<section x-data="{owner: 'receiver', receiverCount: 0}">
    ...
    <c-slot name="empty">
        <c-Button
    c-attrs="{'id': 'fallback', '@click': 'receiverCount += 1'}">fallback</c-Button>
    </c-slot>
...
"""
    ...
    class Child(Component):
        citry = module.app
        template = """
<span id="child" x-data="{owner: 'child'}" x-text="owner"></span>
"""
...
<html><body><main x-data="{owner: 'caller', count: 0}">
    <c-Button
    c-attrs="{'id': 'direct', '@click': 'count += 1'}"><span>direct</span></c-Button>
    <c-receiver><c-fill name="body">
        <c-Button
    c-attrs="{'id': 'fill', '@click': 'count += 1'}"><span>fill</span></c-Button>
    </c-fill></c-receiver>
    <c-Button
    c-attrs="{'id': 'child-wrapper'}"><c-child /></c-Button>
...
def check(browser: Any, url: str, enabled: bool) -> dict[str, Any]:
    ...
    try:
        ...
        scopes = page.evaluate("""() => Object.fromEntries(['direct','fill','fallback','child'].map(id => {
        ...
        expected = {
            name: {
                "owner": ("receiver" if name == "fallback" else "caller") if enabled else None,
                "isolated": not enabled,
            }
            for name in ("direct", "fill", "fallback")
        }
        ...
        if enabled:
            page.locator("#direct span").click()
            page.locator("#fill span").click()
...
def main() -> None:
    ...
    for enabled in (False, True):
        with installed(module, enabled):
            documents["/candidate" if enabled else "/reference"] = str(page_cls())
    results = {}
            ...
            for name in ("chromium", "firefox", "webkit"):
                ...
                try:
                    results[name] = {
                        ...
                        "variants": {
                            variant: check(browser, f"http://127.0.0.1:{server.server_address[1]}/{variant}", enabled)
                            for variant, enabled in (("reference", False), ("candidate", True))
                        },
                    }
                finally:
                    browser.close()

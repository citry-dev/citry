"""Measure default-bound builder values against ordinary closure capture."""
...
from benchmarks.orchestration_probe.adapter import CANDIDATE, ORIGINAL, install  # noqa: E402
...
def capture_counts(module: Any, data: Any, *, changed: bool) -> dict[str, Any]:
    ...
    if components._render_one is not expected:
        raise RuntimeError("The intended render-one implementation is not active")
    return {"snapshot_table_types": tables, "render_one_cellvars": expected.__code__.co_cellvars}
...
def equivalence(size: str = "lg") -> dict[str, Any]:
    ...
    try:
        ...
        if not traces[0] or traces[0] != traces[1] or outputs[0] != outputs[1]:
            raise RuntimeError("Builder capture changed fixture HTML or ownership snapshots")
    finally:
        OwnershipGraph.snapshot = original_snapshot
...
def main() -> None:
    """Retain balanced process comparisons for body-builder capture."""
    ...
    report = {
        ...
        "size": args.size,
        "experiment": "capture body-builder values as defaults without closure cells",

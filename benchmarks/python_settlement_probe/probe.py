"""Compare complete renders with explicit Python settlement state or nested callbacks."""
...
from benchmarks.python_settlement_probe import adapter  # noqa: E402
...
def worker(changed: bool, samples: int, id_base: int) -> dict[str, Any]:
    ...
    metadata = adapter.install(changed)
    module = scenario()
    ...
    state = getattr(runtime, "_SettlementState", None)
    initializer = state.__init__ if state is not None else None

    def observed_init(self: Any, stack: Any, root_result: Any) -> None:
        counts["state_constructions"] += 1
        initializer(self, stack, root_result)

    if state is not None:
        state.__init__ = observed_init
    try:
        ids._id_counter = itertools.count()
        output = module.render(data)
    finally:
        ...
        if state is not None:
            state.__init__ = initializer
    if len(snapshots) != 4 or (changed and not counts["state_constructions"]):
        raise RuntimeError("Missing ownership captures or inactive Python settlement state")
    return {
        "candidate": changed,
        "python": sys.version,

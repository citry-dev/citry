"""Compare complete renders with native serializer sessions or Python frame assembly."""
...
def worker(changed: bool, samples: int, id_base: int) -> dict[str, Any]:
    ...
    try:
        ...
        adapter.COUNTS = None
    if len(snapshots) != 4 or (changed and not counts["frames"]):
        raise RuntimeError("Missing ownership captures or inactive serializer sessions")
    return {
        "candidate": changed,
        "python": sys.version,
        ...
        "candidate_artifact_sha256": hashlib.sha256(adapter.ARTIFACT.read_bytes()).hexdigest(),
        "warmups": warmups,
    ...
    report = {
        ...
        "hashes": {
            ...
            for path in (
                ...
                ROOT / "crates/citry_html_transform/src/marker.rs",
                ROOT / "crates/citry_html_transform/src/transformer.rs",

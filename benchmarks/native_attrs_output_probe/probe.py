"""Measure native flat output-cache keys against nested Python keys."""
...
from benchmarks.native_attrs_output_probe.adapter import ARTIFACT, CANDIDATE, ORIGINAL, install  # noqa: E402
...
def capture_counts(module: Any, data: Any, *, changed: bool) -> dict[str, Any]:
    ...
    counts = {"native": 0, "unsupported": 0}
    original_native = nodes._native_attrs_output_key

    def counted(items: Any) -> Any:
        result = original_native(items)
        counts["unsupported" if result is None else "native"] += 1
        return result

    nodes._native_attrs_output_key = counted
    try:
        module.render(data)
    finally:
        nodes._native_attrs_output_key = original_native
    if changed and not counts["native"]:
        raise RuntimeError("The candidate did not build native output-cache keys")
    return {"snapshot_table_types": tables, "key_calls": counts}
...
def main() -> None:
    ...
    report = {
        ...
        "size": args.size,
        "experiment": "build flat primitive output-cache keys in one native call",

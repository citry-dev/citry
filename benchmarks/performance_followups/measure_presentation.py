"""Compare the qualified presentation group with the released simple benchmark."""
...
def worker(variant: str, samples: int, qualification: dict[str, Any]) -> dict[str, Any]:
    ...
    expected = qualification["combined" if variant == "candidate" else "control"]
    extra = tuple(qualification["eligible"]) if variant == "candidate" else ()
    module, tree_hash = load(extra)
    assert tree_hash == qualification["combined_scenario_sha256" if extra else "control_scenario_sha256"]
    ...
    assert substantive_calls(observed) == substantive_calls(qualification["control"])

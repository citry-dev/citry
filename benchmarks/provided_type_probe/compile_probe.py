"""Measure reuse of constructor compilation while generating fresh payload classes."""
...
def constructor_code(source: str) -> Any:
    """Retain only immutable compiled code for generated constructors."""
    return compile(source, "<string>", "eval", dont_inherit=True)


def cached_eval(source: Any, namespace: dict[str, Any]) -> Any:
    """Create a fresh function in the supplied namespace using bounded compiled code."""
    code = constructor_code(source) if type(source) is str and len(source) <= 4096 else source
    return eval(code, namespace)  # noqa: S307 - trusted standard-library constructor source


def clone(function: Any, **replacements: Any) -> Any:
    """Copy a standard-library factory with experiment-local global replacements."""
    result = FunctionType(
        function.__code__, {**function.__globals__, **replacements}, function.__name__, function.__defaults__
    )
    result.__kwdefaults__ = function.__kwdefaults__
    return result


# Private copies confine the altered eval path to this experiment's payload factory.
CACHED_NAMEDTUPLE = clone(collections.namedtuple, eval=cached_eval)  # noqa: PYI024 - the typed factory delegates here
CACHED_MAKE_NMTUPLE = clone(typing._make_nmtuple, collections=SimpleNamespace(namedtuple=CACHED_NAMEDTUPLE))
CACHED_NAMEDTUPLE_TYPED = clone(typing.NamedTuple, _make_nmtuple=CACHED_MAKE_NMTUPLE)
CANDIDATE = clone(ORIGINAL, NamedTuple=CACHED_NAMEDTUPLE_TYPED)


def install(changed: bool) -> None:
    """Switch the payload factory without patching standard-library modules."""
...
def capture_counts(module: Any, data: Any) -> dict[str, Any]:
    """Count constructor-code reuse during one separate untimed render."""
    before = constructor_code.cache_info()
    module.render(data)
    after = constructor_code.cache_info()
    if after.hits <= before.hits or after.currsize > 128:
        raise RuntimeError("The constructor-code cache was inactive or exceeded its entry bound")
...
def equivalence() -> dict[str, Any]:
    ...
    try:
        ...
        if not traces[0] or traces[0] != traces[1] or outputs[0] != outputs[1]:
            raise RuntimeError("Constructor-code reuse changed fixture HTML or ownership snapshots")
    finally:
        OwnershipGraph.snapshot = original_snapshot
...
def main() -> None:
    """Retain balanced process comparisons for constructor-code reuse."""

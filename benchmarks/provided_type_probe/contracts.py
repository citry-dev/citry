"""Observe unreachable classes and behavior changes from sharing payload types."""
...
from probe import ROOT, ids, install, payload_type, provide_module, scenario
...
def counterexamples(changed: bool) -> dict[str, Any]:
    """Show how one caller's class edit affects a later provided payload."""
    payload_type.cache_clear()
    install(changed)
    try:
        ...
        # The altered experimental class must not enter any subsequent observation.
        payload_type.cache_clear()
        install(changed=False)
...
def main() -> None:
    ...
    if observations["reference"]["html_digest"] != observations["candidate"]["html_digest"]:
        raise RuntimeError("Payload-class reuse changed diagnostic HTML")
    if examples["reference"] == examples["candidate"]:
        raise RuntimeError("The stated payload-class counterexamples changed; review the candidate")
    report = {

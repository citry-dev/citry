"""Measure deferred native ownership allocation against eager allocation."""
...
from benchmarks.deferred_ownership_probe.reference import SOURCE, install  # noqa: E402
...
def capture_counts(module: Any, data: Any, *, changed: bool) -> dict[str, Any]:
    ...
    for table in tables:
        for name, actual in table["types"].items():
            native_expected = not changed or table["nested_invocations"] > 0
    ...
    return {"snapshot_table_types": tables}
...
def main() -> None:
    ...
    report = {
        ...
        "size": args.size,
        "ownership_backend": "deferred allocation with committed eager native runtime as reference",

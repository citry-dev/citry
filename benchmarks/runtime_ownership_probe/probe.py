"""Measure ordinary ownership rendering against its archived Python collector."""
...
def capture_counts(module: Any, data: Any, *, changed: bool) -> dict[str, Any]:
    ...
    for table in tables:
        for name, actual in table["types"].items():
            native_expected = changed and table["nested_invocations"] > 0
...
def main() -> None:
    ...
    report = {
        ...
        "size": args.size,
        "ownership_backend": "ordinary runtime with archived Python collector as reference",

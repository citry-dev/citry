"""Measure packaged compact output-cache keys against the archived formatter."""
...
from benchmarks.attrs_output_qualification.adapter import (  # noqa: E402
    ARTIFACT,
    BUILDER,
    CANDIDATE,
    FALLBACK,
...
def capture_counts(module: Any, data: Any, *, changed: bool) -> dict[str, Any]:
    ...
    expected = CANDIDATE if changed else ORIGINAL
    if nodes.ElementAttrsNode._format is not expected:
        raise RuntimeError("The intended attribute formatter is not active")
    counts = {"admitted": 0, "unsupported": 0}
    original_native = nodes._build_attrs_output_key

    def counted(items: Any) -> Any:
        result = original_native(items)
        counts["unsupported" if result is None else "admitted"] += 1
        return result

    nodes._build_attrs_output_key = counted
    try:
        module.render(data)
    finally:
        nodes._build_attrs_output_key = original_native
    if changed and not counts["admitted"]:
        raise RuntimeError("The candidate did not build output-cache keys")
    if nodes._build_attrs_output_key is not BUILDER:
        raise RuntimeError("The intended key builder is not active")
    return {"snapshot_table_types": tables, "key_calls": counts, "builder": "python" if FALLBACK else "native"}
...
def equivalence(size: str = "lg") -> dict[str, Any]:
    ...
    try:
        ...
        if not traces[0] or traces[0] != traces[1] or outputs[0] != outputs[1]:
            raise RuntimeError("Native output-cache keys changed fixture HTML or ownership snapshots")
    finally:
        OwnershipGraph.snapshot = original_snapshot
...
def main() -> None:
    """Retain balanced process comparisons for native output-cache keys."""
    ...
    report = {
        ...
        "size": args.size,
        "experiment": "use compact keys in the ordinary packaged formatter",
        ...
        "hashes": {
            ...
            for path in (
                ...
                Path(__file__).with_name("reference_format.py.txt"),
                Path(__file__).with_name("reference_line.txt"),
                ROOT / "crates/citry_core_py/src/attrs.rs",
                ROOT / "packages/py/citry_core/citry_core/_attrs.py",
                ROOT / "packages/py/citry_core/tests/test_attrs_output_key.py",
                Path(attrs.__file__),
                Path(nodes.__file__),
                ...
                *sorted((ROOT / "crates/citry_core_py/src/ownership").glob("*.rs")),
                ...
                ROOT / "Cargo.toml",
                ROOT / "Cargo.lock",
                ROOT / "packages/py/citry/tests/test_attrs_output_cache.py",

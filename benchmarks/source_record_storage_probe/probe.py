"""Measure deferred source-record creation in the existing ownership table."""
...
from benchmarks.source_record_storage_probe import adapter  # noqa: E402
...
def capture_counts(module: Any, data: Any, *, changed: bool) -> dict[str, Any]:
    """Inspect actual source storage and hash every full ownership snapshot."""
    original_snapshot = OwnershipGraph.snapshot
    tables, snapshots = [], []
    ids._id_counter = itertools.count()

    def snapshot(graph: Any) -> Any:
        nested = len(graph._component_invocations)
        actual = type(graph._source_locations).__name__
        expected = "RecordTable" if changed and nested else "list"
        if actual != expected:
            raise RuntimeError(f"Unexpected source table {actual}, expected {expected}")
        tables.append({"nested_invocations": nested, "source_records": len(graph._source_locations), "type": actual})
        result = original_snapshot(graph)
        snapshots.append(digest(canonical(result)))
        return result

    expected = adapter.CANDIDATES if changed else adapter.ORIGINALS
    for name, method in expected.items():
        if getattr(OwnershipGraph, name) is not method:
            raise RuntimeError("The intended source-storage adapter is not active")
...
def equivalence(size: str = "lg") -> dict[str, Any]:
    ...
    try:
        ...
        if not traces[0] or traces[0] != traces[1] or outputs[0] != outputs[1]:
            raise RuntimeError("Source storage changed fixture HTML or ownership snapshots")
    finally:
        OwnershipGraph.snapshot = original_snapshot
...
def main() -> None:
    ...
    report = {
        ...
        "size": args.size,
        "experiment": "defer source-record creation using the existing native field table",

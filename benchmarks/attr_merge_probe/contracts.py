"""Compare primitive merges and record helper/identity differences on cache hits."""
...
from probe import ORIGINAL, ROOT, attrs_module, merge_attrs, merge_items
...
def main() -> None:
    ...
    for index in range(400):
        merge_attrs([("data-value", str(index))])
    ...
    if identity != {
        ...
        "candidate_current_key": False,
        "candidate_current_value": False,
    }:
        raise RuntimeError("The stated string-identity counterexample changed")
    ...
    if helper["reference"] == helper["candidate"]:
        raise RuntimeError("The stated changed-helper counterexample changed")
    report = {
        ...
        "string_identity_counterexample": identity,
        "changed_helper_counterexample": helper,
        "hashes": {

"""Measure reuse of attribute-name merge positions with live values."""
...
import citry.attrs as attrs_module  # noqa: E402
...
from citry import nodes, ownership  # noqa: E402
from citry.ownership import OwnershipGraph  # noqa: E402

ORIGINAL = attrs_module._merge_resolved_attrs
IDENTITY_HELPERS = (
    attrs_module._html_attr_identity,
    attrs_module._exact_html_attr_identity,
    attrs_module._uncached_html_attr_identity,
)


@lru_cache(maxsize=256)
def merge_items(keys: tuple[str, ...]) -> Any:
    """Prepare positions for first output keys, last ordinary values and class/style contributions."""
    by_identity = {}
    output = []
    accumulators = {"class": [], "style": []}
    for index, key in enumerate(keys):
        identity = attrs_module._html_attr_identity(key)
        if identity not in by_identity:
            by_identity[identity] = len(output)
            output.append([index, index])
        position = by_identity[identity]
        if identity in accumulators:
            accumulators[identity].append(index)
            output[position][1] = -1
        else:
            output[position][1] = index
    groups = tuple(
        (identity, output[by_identity[identity]][0], tuple(indices))
        for identity, indices in accumulators.items()
        if indices
    )
    return tuple(tuple(pair) for pair in output), groups


def merge_attrs(items: Any) -> dict[str, Any]:
    """Apply cached name positions to current contributions with live normalizers."""
    if (
        type(items) is list
        and len(items) <= 16
        and attrs_module._html_attr_identity is IDENTITY_HELPERS[0]
        and attrs_module._exact_html_attr_identity is IDENTITY_HELPERS[1]
        and attrs_module._uncached_html_attr_identity is IDENTITY_HELPERS[2]
    ):
        ...
        for pair in snapshot:
            ...
            positions, groups = merge_items(tuple(keys))
            result = {snapshot[first][0]: snapshot[last][1] if last >= 0 else None for first, last in positions}
            for identity, first, indices in groups:
                values = [snapshot[index][1] for index in indices if snapshot[index][1] is not None]
                if values:
                    normalizer = (
                        attrs_module._normalize_class_contributions
                        if identity == "class"
                        else attrs_module.normalize_style
                    )
                    result[snapshot[first][0]] = normalizer(values)
            return result
    return ORIGINAL(items)


def install(changed: bool) -> None:
    """Switch the value-layer merger and the alias used by element nodes."""
    factory = merge_attrs if changed else ORIGINAL
    attrs_module._merge_resolved_attrs = factory
    nodes._merge_resolved_attrs = factory


def capture_counts(module: Any, data: Any) -> dict[str, Any]:
    """Count cache hits during one separate untimed render."""
    before = merge_items.cache_info()
    module.render(data)
    after = merge_items.cache_info()
    if after.hits <= before.hits or after.currsize > 256:
        raise RuntimeError("The merged-attribute cache was inactive or exceeded its entry bound")
...
def equivalence() -> dict[str, Any]:
    ...
    try:
        ...
        if not traces[0] or traces[0] != traces[1] or outputs[0] != outputs[1]:
            raise RuntimeError("Attribute-position reuse changed fixture HTML or ownership snapshots")
    finally:
        OwnershipGraph.snapshot = original_snapshot
...
def main() -> None:
    """Retain balanced process comparisons for attribute merge positions."""

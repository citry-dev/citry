...
def merge_items(items: tuple[tuple[str, Any], ...]) -> tuple[tuple[str, Any], ...]:
    """Retain immutable merged items under the experiment's original helpers."""
    return tuple(ORIGINAL(items).items())


def merge_attrs(items: Any) -> dict[str, Any]:
    """Snapshot bounded primitive inputs and return a fresh dictionary on hits."""
    if type(items) is list and len(items) <= 16:
        ...
        for pair in snapshot:
            ...
            return dict(merge_items(snapshot))
...
def equivalence() -> dict[str, Any]:
    ...
    try:
        ...
        if not traces[0] or traces[0] != traces[1] or outputs[0] != outputs[1]:
            raise RuntimeError("Merged-attribute reuse changed fixture HTML or ownership snapshots")
    finally:
        OwnershipGraph.snapshot = original_snapshot
...
def main() -> None:
    """Retain balanced process comparisons for merged attribute reuse."""

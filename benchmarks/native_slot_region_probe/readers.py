...
def main() -> None:
    ...
    try:
        ...
        if sum(counts.values()) != 1811 or any(caller in {"capture", "capture_slot_call"} for _, caller in counts):
            raise RuntimeError("Native slot preparation did not remove the expected intermediate exports")
    finally:
        RESTORE()
    report = {

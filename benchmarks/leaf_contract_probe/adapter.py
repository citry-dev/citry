...
def install(module: Any, enabled: bool, *, variant: str = "contract") -> None:
    """Select one trusted fixture class; leave the ordinary runtime installed otherwise."""
    if variant not in ("reuse", "contract"):
        raise ValueError("Unknown icon contract variant")
    ...
    def render(element: Any, parent: Any = None, provides: Any = None) -> Any:
        ...
        if variant == "reuse":
            ...
            # plain() already checks every input value. Only attribute names add
            # another restriction; the application's output is trusted by contract.
        ...
        # Reuse the existing render-local keys and plans. The selected contract

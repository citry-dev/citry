...
def main() -> None:
    ...
    try:
        ...
        if sum(counts.values()) != 1727 or any(
            caller in {"source", "bind_template_fill_sources", "bind_supplied_slots", "_revive_retired_template_fill"}
            for _, caller in counts
        ):
            raise RuntimeError("Grouped fill binding did not remove the expected intermediate exports")
    finally:
        RESTORE()
    report = {

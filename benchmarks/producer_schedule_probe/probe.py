"""Compare complete renders with producer-supplied scheduling or ordinary discovery."""
...
def worker(changed: bool, samples: int, id_base: int) -> dict[str, Any]:
    ...
    import citry.component_render as cr  # noqa: PLC0415
    ...
    scan_parts = cr._scan_deferred_parts
    ...
    def observed_consume(plan: Any, tasks: Any) -> bool:
        counts["plans_consumed"] += 1
        counts["planned_entries"] += len(plan.entries)
        counts["planned_input_lengths"] += len(plan.parts)
        return consume(plan, tasks)

    def observed_scan(parts: Any, context: Any, tasks: Any) -> bool:
        counts["ordinary_scan_calls"] += 1
        counts["ordinary_scan_input_lengths"] += len(parts)
        return scan_parts(parts, context, tasks)

    OwnershipGraph.snapshot = observed_snapshot
    adapter.consume = observed_consume
    cr._scan_deferred_parts = observed_scan
    try:
        ids._id_counter = itertools.count()
        output = module.render(data)
    finally:
        ...
        cr._scan_deferred_parts = scan_parts
    if len(snapshots) != 4 or (changed and not counts["plans_consumed"]):
        raise RuntimeError("Missing ownership captures or inactive scheduling plans")
    if adapter.SCOPE.get() is not None:
        raise RuntimeError("A scheduling scope escaped rendering")
    return {
        "candidate": changed,
        "python": sys.version,
